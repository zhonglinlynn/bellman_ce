...
impl<E: Engine> Crs<E, CrsForLagrangeForm> {
    ...
    pub fn from_powers(powers: &Crs<E, CrsForMonomialForm>, size: usize, worker: &Worker) -> Self {
        ...
        let g1 = g1
            .into_iter()
            .map(|el| Point(el.into_projective()))
            .collect();
...
pub(crate) fn elements_into_representations<E: Engine>(
    ...
    let mut representations: Vec<<E::Fr as PrimeField>::Repr> = Vec::with_capacity(size);
    unsafe {
        representations.set_len(size);
    }
...
pub fn commit_using_monomials<E: Engine>(
    ...
    let length = scalars_repr.len();
    let exps = Arc::new(scalars_repr);

    println!("Scalars conversion taken {:?}", subtime.elapsed());

    let subtime = Instant::now();
    let bases = Arc::new(crs.g1_bases[..length].to_vec());

    let res = multiexp::dense_multiexp::<E::G1Affine>(&worker, bases, exps, kern);
    let res = res.wait()?;
...
pub fn commit_using_values<E: Engine>(
    ...
    let bases = Arc::new(crs.g1_bases[..].to_vec());
    let res = multiexp::dense_multiexp::<E::G1Affine>(&worker, bases, exps, kern);
    let res = res.wait()?;
...
pub fn commit_using_raw_values<E: Engine>(
    ...
    let exps = Arc::new(scalars_repr);
    let bases = Arc::new(crs.g1_bases[..values.len()].to_vec());
...
pub fn open_from_monomials<E: Engine>(
    ...
    let opening_proof = commit_using_monomials(&division_result, &crs, &worker, kern)?;
...
pub fn open_from_values<E: Engine>(
    ...
    let mut division_result: Polynomial<E, Values> = Polynomial::from_values(division_result)?;
    ...
    worker.scope(division_result.size(), |scope, chunk_size| {
        for (result, values) in division_result
            .as_mut()
    ...
    let opening_proof = commit_using_values(&division_result, &crs, &worker, kern)?;
...
pub fn open_from_values_on_coset<E: Engine>(
    ...
    let mut division_result: Polynomial<E, Values> = Polynomial::from_values(division_result)?; // [g, g, g, g, ...]
    ...
    let opening_proof = commit_using_values_on_coset(&division_result, &crs, &worker, kern)?;

    Ok(opening_proof)
}

pub fn perform_batched_divisor_for_opening<E: Engine>(
    mut polynomials: Vec<Polynomial<E, Values>>,
    open_at: E::Fr,
    opening_values: &[E::Fr],
    challenge: E::Fr,
    challenge_start: E::Fr,
    worker: &Worker,
) -> Result<(Polynomial<E, Values>, E::Fr), SynthesisError> {
    assert!(
        polynomials.len() == opening_values.len(),
        "different number of polynomials and opening values"
    );
...
pub fn perform_batch_opening_from_values<E: Engine>(
    polynomials: Vec<Polynomial<E, Values>>,
    crs: &Crs<E, CrsForLagrangeForm>,
    open_at: E::Fr,
    opening_values: &[E::Fr],
    challenge: E::Fr,
    ...
    let opening_proof = commit_using_values(&aggregation, &crs, &worker, kern)?;
...
pub fn is_valid_opening<E: Engine>(
    ...
    let result = E::final_exponentiation(&E::miller_loop(&[
        (
            &pair_with_1_part.into_affine().prepare(),
...
pub fn is_valid_multiopening<E: Engine>(
    ...
    let result = E::final_exponentiation(&E::miller_loop(&[
        (
            &aggregation.into_affine().prepare(),
            &E::G2Affine::one().prepare(),
        ),
        (&pair_with_x_part.prepare(), &g2_by_x.prepare()),
    ]));
...
pub(crate) mod test {
    ...
    fn test_transformations_of_crs_2() {
        ...
        let poly = Polynomial::<Bn256, Coefficients>::from_coeffs(vec![one, two]).unwrap();
        let values = poly.clone().fft(&worker, &mut None);
        let values_on_coset = poly.clone().coset_fft(&worker, &mut None);
        ...
        let commitment = commit_using_monomials(&poly, &monomial, &worker, &mut None).unwrap();
        let commitment_values =
            commit_using_values(&values, &lagrange, &worker, &mut None).unwrap();
        let commitment_values_on_coset =
            commit_using_values_on_coset(&values_on_coset, &lagrange_coset, &worker, &mut None)
                .unwrap();
    ...
    fn test_transformations_of_crs_4() {
        ...
        let poly =
            Polynomial::<Bn256, Coefficients>::from_coeffs(vec![one, two, one, two]).unwrap();
        let values = poly.clone().fft(&worker, &mut None);
        let values_on_coset = poly.clone().coset_fft(&worker, &mut None);
    ...
    fn test_transformations_of_crs_large() {
        ...
        let poly = Polynomial::<Bn256, Coefficients>::from_coeffs(vec![two; size]).unwrap();
        let values = poly.clone().fft(&worker, &mut None);
        let values_on_coset = poly.clone().coset_fft(&worker, &mut None);
    ...
    fn test_opening_large() {
        ...
        let valus_on_coset_at_z = values_on_coset
            .barycentric_over_coset_evaluate_at(&worker, z, &Fr::multiplicative_generator())
            .unwrap();
        ...
        let opening_poly =
            open_from_monomials(&poly, z, poly_at_z, &monomial, &worker, &mut None).unwrap();
        let opening_values =
            open_from_values(&values, z, poly_at_z, &lagrange, &worker, &mut None).unwrap();
        let opening_values_on_coset = open_from_values_on_coset(
            &values_on_coset,
        ...
        let valid = is_valid_opening::<Bn256>(
            commitment,
            z,
            poly_at_z,
            opening_poly,
    ...
    fn transform_ignition_setup() {
        ...
        for n in 20..=26 {
            ...
            let lagrange =
                Crs::<Bn256, CrsForLagrangeForm>::from_powers(&monomial_form, size, &worker);
    ...
    fn test_crs_serialization() {
        ...
        let new =
            Crs::<Bn256, CrsForMonomialForm>::read(&buffer[..]).expect("must deserialize CRS");
    ...
    pub(crate) fn make_random_field_elements_for_rng<F: PrimeField, R: Rng + Send>(
        worker: &Worker,
        num_elements: usize,
        mut rng: R,
    ) -> Vec<F> {
    ...
    fn make_random_g1_points_for_rng<G: CurveAffine, R: Rng + Send>(
        worker: &Worker,
        num_elements: usize,
        mut rng: R,
    ) -> Vec<G> {
    ...
    fn test_multiexp_performance_on_large_data() {
        ...
        assert!(
            worker.get_num_cpus() >= 16,
            "should be tested only on large machines"
        );
        ...
        for size in vec![1 << 23, 1 << 24, 1 << 25, 1 << 26] {
            for cpus in vec![16, 32, 48, 64] {
                ...
                let s = &scalars[..size];
                let g = Arc::new(points[..size].to_vec());
                ...
                let scalars_repr =
                    super::elements_into_representations::<Bn256>(&subworker, s).unwrap();
                let exps = Arc::new(scalars_repr);
                ...
                let res = multiexp::dense_multiexp::<<Bn256 as Engine>::G1Affine>(
                    &subworker, g, exps, &mut None,
                );
                ...
                println!(
                    "Total time taken for {} points on {} cpus = {:?}",
                    size,
                    cpus,
                    now.elapsed()
                );
    ...
    //     let worker = Worker::new();
    //
    //     assert!(worker.get_num_cpus() >= 16, "should be tested only on large machines");
    //     println!("Generating scalars");
    //     let scalars = make_random_field_elements::<Fr>(&worker, max_size);
    //     println!("Generating points");
    //     let points = make_random_g1_points::<<Bn256 as Engine>::G1Affine>(&worker, max_size);
    ...
    fn test_long_naive_division() {
        ...
        for size in vec![1 << 23, 1 << 24, 1 << 25, 1 << 26] {
            ...
            println!(
                "Total time taken for {} points division = {:?}",
                size,
                now.elapsed()
            );
        }
    ...
    fn serialize_projective_points_for_fpga<E: Engine, W: std::io::Write>(
        ...
        println!(
            "Second bucket (for scalar = 1): X = {}, Y = {}, Z = {}",
            x, y, z
        );
        for p in points.iter() {
    ...
    fn simulate_first_buckets<E: Engine>(
        points: &[E::G1Affine],
        scalars: &[E::Fr],
        c: usize,
        random_point: E::G1Affine,
    ) -> Vec<E::G1> {
        use crate::pairing::ff::PrimeFieldRepr;
    ...
    //
    //             let scalars_repr =
    //                 super::elements_into_representations::<E>(&subworker, s).unwrap();
    //
    //             let subtime = Instant::now();
    //
    //             let _ =
    //                 multiexp::dense_multiexp::<E::G1Affine>(&subworker, &g, &scalars_repr).unwrap();
    //
    //             println!(
    //                 "Dense simple multiexp of size {} taken {:?} on {} cpus",
    //                 size,
    //                 subtime.elapsed(),
    //                 cpus
    //             );
    ...
    // fn test_future_based_multiexps_over_window_sizes<E: Engine>(
    //     max_size: usize,
    //     sizes: Vec<usize>,
    //     num_cpus: Vec<usize>,
    //     windows: Vec<usize>,
    ...
    //
    //             println!(
    //                 "Standard future based multiexp of size {} on {} CPUs:",
    //                 size, cpus
    //             );
    //             for (window, time_ms) in &subresults[0..3] {
    //                 println!("Window = {}, time = {} ms", window, time_ms);
    ...
    //
    //             println!(
    //                 "Tuned future based multiexp of size {} on {} CPUs:",
    //                 size, cpus
    //             );
    ...
    fn produce_fpga_test_vectors() {
        ...
        println!(
            "Random point in Montgomery form: X = {}, Y = {}",
            x.into_raw_repr(),
            y.into_raw_repr()
        );
    ...
    fn produce_bn254_fpga_test_vectors() {
        ...
        for n in vec![6, 7, 20] {
            let points_path = base_path.join(&format!(
                "bn_254_input_points_2^{}_width_{}.key",
                n, bucket_width
            ));
            let scalars_path = base_path.join(&format!(
                "bn_254_input_scalars_2^{}_width_{}.key",
                n, bucket_width
            ));
            let initial_buckets_path = base_path.join(&format!(
                "bn_254_input_buckets_2^{}_width_{}.key",
                n, bucket_width
            ));
            let buckets_path = base_path.join(&format!(
                "bn_254_output_buckets_2^{}_width_{}.key",
                n, bucket_width
            ));
            ...
            let initial_buckets =
                vec![random_point.into_projective(); num_buckets * (1 << bucket_width)];
            let buckets =
                simulate_first_buckets::<Bn256>(&points, &scalars, bucket_width, random_point);
            ...
            serialize_projective_points_for_fpga::<Bn256, _>(
                &initial_buckets,
                &mut initial_buckets_file,
            )
            .unwrap();
    ...
    fn produce_fpga_window_12_test_vectors() {
        ...
        for n in vec![6, 7, 20] {
            ...
            let scalars_path = base_path.join(&format!("input_scalars_2^{}.key", n));
            let buckets_path =
                base_path.join(&format!("width_{}_output_buckets_2^{}.key", width, n));
