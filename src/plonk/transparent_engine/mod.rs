...
#[cfg(test)]
mod test {
    use super::proth_engine::Transparent252 as Transparent252Optimized;
    use super::Transparent252;
    #[test]
        ...
        for poly_size in poly_sizes.into_iter() {
            let res1 = {
                ...
                let coeffs = (0..poly_size)
                    .map(|_| FrMontNaive::rand(rng))
                    .collect::<Vec<_>>();

                let poly = Polynomial::<Transparent252, _>::from_coeffs(coeffs).unwrap();
                let start = Instant::now();
                let eval_result = poly.lde(&worker, 16).unwrap();
                println!(
                    "LDE with factor 16 for size {} with naive mont reduction taken {:?}",
                    poly_size,
                    start.elapsed()
                );
            ...
            let res2 = {
                let rng =
                    &mut XorShiftRng::from_seed([0x3dbe6259, 0x8d313d76, 0x3237db17, 0xe5bc0654]);
                let coeffs = (0..poly_size)
                    .map(|_| FrOptimized::rand(rng))
                    .collect::<Vec<_>>();

                let poly = Polynomial::<Transparent252Optimized, _>::from_coeffs(coeffs).unwrap();
                let start = Instant::now();
                let eval_result = poly.lde(&worker, 16).unwrap();
                println!(
                    "LDE with factor 16 for size {} with optimized mont reduction taken {:?}",
                    poly_size,
                    start.elapsed()
                );
    ...
    fn test_proth_field() {
        ...
        assert_eq!(
            to_hex(&FrMontNaive::multiplicative_generator()),
            to_hex(&FrOptimized::multiplicative_generator())
        );
    }
    ...
    fn test_bench_precomputations_for_proth_fft() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            let precomp =
                PrecomputedOmegas::<FrOptimized>::new_for_domain_size(domain.size as usize);
            ...
            let res1 = {
                let rng =
                    &mut XorShiftRng::from_seed([0x3dbe6259, 0x8d313d76, 0x3237db17, 0xe5bc0654]);
                let mut coeffs = (0..poly_size)
                    .map(|_| FrOptimized::rand(rng))
                    .collect::<Vec<_>>();
                let start = Instant::now();
                best_fft(&mut coeffs, &worker, &omega, log_n, None);
                println!(
                    "FFT for size {} without precomputation taken {:?}",
                    poly_size,
                    start.elapsed()
                );
            ...
            let res2 = {
                ...
                println!(
                    "FFT for size {} with precomputation taken {:?}",
                    poly_size,
                    start.elapsed()
                );
    ...
    fn test_bench_precomputations_for_proth_lde() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            let res2 = {
                ...
                let start = Instant::now();
                let eval_result = poly
                    .lde_using_multiple_cosets_with_precomputation(&worker, 16, &precomp)
                    .unwrap();
    ...
    fn test_bench_ct_ploth_lde() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            let res2 = {
                ...
                let start = Instant::now();
                let eval_result = poly
                    .lde_using_bitreversed_ntt(&worker, 16, &precomp)
                    .unwrap();
                println!(
                    "LDE with factor 16 for size {} with optimized ntt taken {:?}",
                    poly_size,
                    start.elapsed()
                );
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            let mut res2 = {
                ...
                let eval_result = poly
                    ...
                    .unwrap();
                println!(
                    "LDE with factor 16 for size {} bitreversed {:?}",
                    poly_size,
                    start.elapsed()
                );
    ...
    fn test_bench_ct_proth_fft() {
        ...
        use crate::plonk::fft::cooley_tukey_ntt::{
            best_ct_ntt, BitReversedOmegas, CTPrecomputations,
        };
        use crate::plonk::fft::fft::best_fft;
        use crate::plonk::polynomials::*;
        use crate::worker::Worker;
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            for i in 0..=20 {
                ...
                println!(
                    "Omega^{} = {}",
                    pow,
                    domain.generator.pow(&[pow]).into_raw_repr()
                );
            }
