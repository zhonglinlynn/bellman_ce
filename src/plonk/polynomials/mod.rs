...
impl<E: Engine> Polynomial<E, Coefficients> {
    ...
    pub fn lde_using_bitreversed_ntt<P: CTPrecomputations<E::Fr>>(
        ...
        // let res_ptr = &mut final_values[..] as *mut [E::Fr];
    ...
    // pub fn lde_using_bitreversed_ntt_no_allocations_lowest_bits_reversed<P: CTPrecomputations<E::Fr>>(
    //     self,
    //     worker: &Worker,
    //     factor: usize,
    //     precomputed_omegas: &P
...
mod test {
    ...
    fn test_insane_size_lde() {
        ...
        for size in vec![1 << 23, 1 << 24, 1 << 25, 1 << 26] {
            ...
            for cpus in vec![4, 8, 16, 32] {
                ...
                println!(
                    "Total LDE time for {} points on {} cpus = {:?}",
                    size,
                    cpus,
                    now.elapsed()
                );
    ...
    fn test_fft_scaling() {
        ...
        use crate::multicore_gpu::Worker;
        use rand::{Rand, Rng, SeedableRng, XorShiftRng};
        ...
        for size in vec![1 << 23, 1 << 24, 1 << 25, 1 << 26] {
            ...
            for cpus in vec![4, 8, 16, 32] {
                ...
                println!(
                    "Total FFT time time for {} points on {} cpus = {:?}",
                    size,
                    cpus,
                    now.elapsed()
                );
