...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> ProverAssembly<E, P> {
    ...
    pub fn make_witness_polynomials(
        ...
        let new_size = (self.n + 1).next_power_of_two() - 1;
        let input_size = self.num_inputs;
        let wire_size = self.wire_assignments[0].len();

        let mut full_assignments = vec![];

        for _ in 0..self.wire_assignments.len() {
            let mut assignments: Vec<E::Fr> = Vec::with_capacity(new_size);
            unsafe { assignments.set_len(new_size) };

            full_assignments.push(assignments);
        }

        for (i, inp) in self.input_assingments.iter().enumerate() {
            // inputs will always go into A wire
            full_assignments[0][i] = *inp;
        }

        let cpus = worker.get_num_cpus() / 4;
        let chunk = Worker::chunk_size_for_num_spawned_threads(wire_size, cpus);
        worker.in_place_scope(wire_size, |scope, _| {
            for (wire, full) in self
                .wire_assignments
                .iter()
                .zip(full_assignments.iter_mut())
            {
                for (j, (wire_chunk_j, full_chunk_j)) in wire
                    .chunks(chunk)
                    .zip(full[input_size..input_size + wire_size].chunks_mut(chunk))
                    .enumerate()
                {
                    scope.spawn(move |_| {
                        let start = j * chunk;
                        let end = if start + chunk <= wire_size {
                            start + chunk
                        } else {
                            wire_size
                        };
                        let copy_start_pointer: *mut E::Fr = full_chunk_j.as_mut_ptr();
...
impl<E: Engine> ProverAssembly4WithNextStep<E> {
    ...
    >(
        ...
        let z_commitment = commit_using_values(&z, &crs_vals, &worker, &mut None)?;
        ...
        for t_part in t_poly_parts.iter() {
            let t_part_commitment = commit_using_monomials(&t_part, &crs_mon, &worker, &mut None)?;
        ...
        let opening_at_z = commit_using_monomials(&open_at_z, &crs_mon, &worker, &mut None)?;

        let opening_at_z_omega =
            commit_using_monomials(&open_at_z_omega, &crs_mon, &worker, &mut None)?;
