...
pub struct SetupPolynomials<E: Engine, P: PlonkConstraintSystemParams<E>> {
    ...
    pub selector_polynomials: Vec<Polynomial<E, Coefficients>>,
    pub next_step_selector_polynomials: Vec<Polynomial<E, Coefficients>>,
    pub permutation_polynomials: Vec<Polynomial<E, Coefficients>>,
...
pub fn read_optional_curve_affine<G: CurveAffine, R: Read>(
    mut reader: R,
) -> std::io::Result<Option<G>> {
...
pub fn write_optional_curve_affine<G: CurveAffine, W: Write>(
    p: &Option<G>,
    mut writer: W,
) -> std::io::Result<()> {
...
pub fn write_curve_affine_vec<G: CurveAffine, W: Write>(
    p: &[G],
    mut writer: W,
) -> std::io::Result<()> {
...
pub fn write_optional_fr<F: PrimeField, W: Write>(
    el: &Option<F>,
    mut writer: W,
) -> std::io::Result<()> {
    use crate::ff::PrimeFieldRepr;
...
pub fn read_optional_polynomial_coeffs<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Option<Polynomial<E, Coefficients>>> {
    let is_some = read_optional_flag(&mut reader)?;
    if is_some {
...
pub fn read_optional_polynomial_values_unpadded<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Option<Polynomial<E, Values>>> {
    let is_some = read_optional_flag(&mut reader)?;
    if is_some {
...
pub fn write_optional_polynomial<E: Engine, P: PolynomialForm, W: Write>(
    p: &Option<Polynomial<E, P>>,
    mut writer: W,
) -> std::io::Result<()> {
    write_optional_flag(p.is_some(), &mut writer)?;
    if let Some(p) = p.as_ref() {
...
pub fn read_polynomials_coeffs_vec<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Vec<Polynomial<E, Coefficients>>> {
...
pub fn read_polynomials_values_unpadded_vec<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Vec<Polynomial<E, Values>>> {
    let num_polys = reader.read_u64::<BigEndian>()?;
    let mut polys = vec![];
    for _ in 0..num_polys {
        let p = read_polynomial_values_unpadded(&mut reader)?;
...
pub fn write_polynomials_vec<E: Engine, P: PolynomialForm, W: Write>(
    p: &[Polynomial<E, P>],
    mut writer: W,
) -> std::io::Result<()> {
    writer.write_u64::<BigEndian>(p.len() as u64)?;
...
pub fn write_polynomial<E: Engine, P: PolynomialForm, W: Write>(
    p: &Polynomial<E, P>,
    mut writer: W,
) -> std::io::Result<()> {
...
pub fn read_polynomial_coeffs<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Polynomial<E, Coefficients>> {
...
pub fn read_polynomial_values_unpadded<E: Engine, R: Read>(
    mut reader: R,
) -> std::io::Result<Polynomial<E, Values>> {
    let num_values = reader.read_u64::<BigEndian>()?;
...
pub struct SetupPolynomialsPrecomputations<E: Engine, P: PlonkConstraintSystemParams<E>> {
    pub selector_polynomials_on_coset_of_size_4n_bitreversed: Vec<Polynomial<E, Values>>,
    pub next_step_selector_polynomials_on_coset_of_size_4n_bitreversed: Vec<Polynomial<E, Values>>,
    pub permutation_polynomials_on_coset_of_size_4n_bitreversed: Vec<Polynomial<E, Values>>,
    pub permutation_polynomials_values_of_size_n_minus_one: Vec<Polynomial<E, Values>>,
    pub inverse_divisor_on_coset_of_size_4n_bitreversed: Polynomial<E, Values>,
    pub x_on_coset_of_size_4n_bitreversed: Polynomial<E, Values>,
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> SetupPolynomialsPrecomputations<E, P> {
    ...
    //     let mut log_d = 0;
    //     let n = required_domain_size * LDE_FACTOR;
    //     while (1 << log_d) < n {
    //         log_d += 1;
    //     }
    //
    ...
    pub fn from_setup(
        ...
        vanishing_poly_inverse_bitreversed.batch_inversion(&worker)?;
        let new_size = vanishing_poly_inverse_bitreversed.size();

        let vanishing_poly_inverse_bitreversed = {
        ...
        // evaluate polynomial X on the coset
        let mut x_poly: Polynomial<E, Values> = Polynomial::from_values(vec![
            coset_generator;
            vanishing_poly_inverse_bitreversed.size()
        ])?;
        x_poly.distribute_powers(&worker, x_poly.omega);
    ...
    pub fn write<W: Write>(&self, mut writer: W) -> std::io::Result<()> {
        writer.write_u64::<BigEndian>(
            self.selector_polynomials_on_coset_of_size_4n_bitreversed
                .len() as u64,
        )?;
        ...
        writer.write_u64::<BigEndian>(
            self.next_step_selector_polynomials_on_coset_of_size_4n_bitreversed
                .len() as u64,
        )?;
        ...
        writer.write_u64::<BigEndian>(
            self.permutation_polynomials_on_coset_of_size_4n_bitreversed
                .len() as u64,
        )?;
        ...
        writer.write_u64::<BigEndian>(
            self.permutation_polynomials_values_of_size_n_minus_one
                .len() as u64,
        )?;
        ...
        write_polynomial(
            &self.inverse_divisor_on_coset_of_size_4n_bitreversed,
            &mut writer,
        )?;
    ...
    pub fn read<R: Read>(mut reader: R) -> std::io::Result<Self> {
        ...
        let inverse_divisor_on_coset_of_size_4n_bitreversed =
            read_polynomial_values_unpadded(&mut reader)?;
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> VerificationKey<E, P> {
    pub fn from_setup(
        ...
        for p in setup.selector_polynomials.iter() {
            let commitment = commit_using_monomials(p, &crs, &worker, &mut multiexp_kern)?;
