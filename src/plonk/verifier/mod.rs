...
fn evaluate_lagrange_poly<E: Engine>(
    ...
    let domain = Domain::<E::Fr>::new_for_size(vahisning_size as u64)
        .expect("domain of this size should exist");
...
>(
    ...
    let mut inverse_vanishing_at_z =
        evaluate_inverse_vanishing_poly::<E>(required_domain_size.next_power_of_two(), z);
    ...
    let l_n_minus_one_at_z =
        evaluate_lagrange_poly::<E>(required_domain_size.next_power_of_two(), setup.n - 1, z);
    ...
    let valid = large_committer.verify_single(
        &proof.t_commitment,
        z,
        proof.t_opening_value,
        &proof.t_opening_proof,
        &mut transcript,
    );
...
pub fn verify_nonhomomorphic_chunked<
    E: Engine,
    S: CommitmentScheme<E::Fr, Prng = T>,
    T: Transcript<E::Fr, Input = S::Commitment>,
>(
...
mod test {
    ...
    fn test_small_circuit_transparent_verification() {
        ...
        let proof = prove_nonhomomorphic::<Bn256, Committer, Blake2sTranscript<Fr>, _>(
            &circuit,
            &setup,
            &aux,
            meta.clone(),
        ...
        let valid = verify_nonhomomorphic::<Bn256, Committer, Blake2sTranscript<Fr>>(
            &setup, &proof, meta, meta_large,
        )
        .unwrap();
    ...
    fn test_bench_fibonacci_circuit() {
        ...
        .unwrap();
        println!(
            "Verification with unnecessary precomputation taken {:?}",
            start.elapsed()
        );
    ...
    fn test_bench_homomorphic_plonk() {
        ...
        println!(
            "{} ms for expected plonk with ~ {} gates",
            total_expected_plonk / 1000u128,
            SAMPLES
        );
    }
    ...
    fn test_bench_transparent_engine() {
        ...
        println!("Start proving");
        let start = Instant::now();
        let proof = prove_nonhomomorphic::<Transparent252, Committer, Blake2sTranscript<Fr>, _>(
            &circuit,
            &setup,
            &aux,
            meta.clone(),
        ...
        println!("Start verifying");
        let start = Instant::now();
        let valid = verify_nonhomomorphic::<Transparent252, Committer, Blake2sTranscript<Fr>>(
            &setup, &proof, meta, meta_large,
        )
        .unwrap();
        println!(
    ...
    fn test_bench_chunked_proof_on_transparent_engine() {
        ...
        println!("Start proving");
        let start = Instant::now();
        let proof = prove_nonhomomorphic_chunked::<
            Transparent252,
            Committer,
            Blake2sTranscript<Fr>,
            _,
        >(&circuit, &aux, meta.clone())
        .unwrap();
        println!("Proof taken {:?}", start.elapsed());
        ...
        let start = Instant::now();
        let valid =
            verify_nonhomomorphic_chunked::<Transparent252, Committer, Blake2sTranscript<Fr>>(
                &setup, &proof, meta,
            )
            .unwrap();
        println!(
    ...
    fn test_poly_grand_product_correctness() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            if palallel_result != serial_result {
                for (i, (c0, c1)) in palallel_result
                    .as_ref()
                    .iter()
                    ...
                    .enumerate()
                {
                    assert!(
                        c0 == c1,
                        "failed at value number {} for size {}",
                        i,
                        poly_size
                    );
                }
    ...
    fn test_bench_lde() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            println!(
                "LDE with factor 16 for size {} taken {:?}",
                poly_size,
                start.elapsed()
            );

            let coeffs = (0..(16 * poly_size))
                .map(|_| Fr::rand(rng))
                .collect::<Vec<_>>();
