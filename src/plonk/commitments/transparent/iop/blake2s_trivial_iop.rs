...
impl<F: PrimeField> IopTreeHasher<F> for Blake2sTreeHasher<F> {
    ...
    fn hash_encoded_leaf(
        value: &<Self::LeafEncoder as LeafEncoder<F>>::Output,
    ) -> Self::HashOutput {
...
impl<'a, F: PrimeField> IopTree<F> for Blake2sIopTree<F> {
    ...
    fn create(leafs: &[F]) -> Self {
                ...
                for (i, lh) in leaf_hashes.chunks_mut(chunk).enumerate() {
                    scope.spawn(move |_| {
                        ...
                        for (j, lh) in lh.iter_mut().enumerate() {
                            let idx = base_idx + j;
                            let leaf_ref = <Self::Combiner as CosetCombiner<F>>::get_for_tree_index(
                                &leafs, idx,
                            );
        ...
        println!(
            "Done creating a tree of size {} in {:?}",
            leafs.len(),
            start.elapsed()
        );
    ...
    fn verify(
        root: &<Self::TreeHasher as IopTreeHasher<F>>::HashOutput,
        leaf_value: &F,
        path: &[<Self::TreeHasher as IopTreeHasher<F>>::HashOutput],
        tree_index: usize,
    ) -> bool {
    ...
    fn get_path(
        ...
        tree_index: usize,
        leafs_values: &[F],
    ) -> Vec<<Self::TreeHasher as IopTreeHasher<F>>::HashOutput> {
...
impl<F: PrimeField> IOP<F> for TrivialBlake2sIOP<F> {
    ...
    fn verify_query(
        query: &Self::Query,
        root: &<<Self::Tree as IopTree<F>>::TreeHasher as IopTreeHasher<F>>::HashOutput,
    ) -> bool {
