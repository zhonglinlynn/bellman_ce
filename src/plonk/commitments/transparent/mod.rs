...
pub struct StatelessTransparentCommitter<
    E: Engine,
    FRI: FriIop<E>,
    T: Transcript<E::Fr, Input = < < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Tree as IopTree<E::Fr> >::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput >
>{
    ...
    num_queries: usize,
    worker: Worker,
    precomputed_inverse_omegas: PrecomputedInvOmegas<E::Fr>,
    precomputed_bitreversed_omegas: BitReversedOmegas<E::Fr>,
    fri_params: <FRI as FriIop<E>>::Params,
...
impl<
    ...
    FRI: FriIop<E>,
    T: Transcript<E::Fr, Input = < < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Tree as IopTree<E::Fr> >::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput >
> std::fmt::Debug for StatelessTransparentCommitter<E, FRI, T> {
...
#[derive(Debug)]
pub struct TransparentCommitterParameters<E: Engine, FRI: FriIop<E>>{
    pub lde_factor: usize,
    pub num_queries: usize,
    pub output_coeffs_at_degree_plus_one: usize,
    pub fri_params: <FRI as FriIop<E>>::Params,
}

impl<E: Engine, FRI: FriIop<E>> Clone for TransparentCommitterParameters<E, FRI> {
    fn clone(&self) -> Self {
        TransparentCommitterParameters::<E, FRI>{
...
impl<
    ...
    FRI: FriIop<E>,
    T: Transcript<E::Fr, Input = < < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Tree as IopTree<E::Fr> >::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput >
> CommitmentScheme<E> for StatelessTransparentCommitter<E, FRI, T> {
    type Commitment = < < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Tree as IopTree<E::Fr> >::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput;
    type OpeningProof = (FRI::Proof, Vec<Vec< (E::Fr, < < FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Query) > >);
    type IntermediateData = (Polynomial<E, Values>, < FRI as FriIop<E> >::IopType);
    type Meta = TransparentCommitterParameters<E, FRI>;
    type Prng = T;
    ...
    fn new_for_size(max_degree_plus_one: usize, meta: Self::Meta) -> Self {
        ...
        let base_domain = Domain::<E::Fr>::new_for_size(base_size as u64).expect("domain of large enough size should exist");
        let lde_domain = Domain::<E::Fr>::new_for_size(lde_domain_size as u64).expect("domain of large enough size should exist");
        let worker = Worker::new();
        let omegas_inv_precomp = PrecomputedInvOmegas::<E::Fr>::new_for_domain(&lde_domain, &worker);
        let omegas_bitrev_precomp = BitReversedOmegas::<E::Fr>::new_for_domain(&base_domain, &worker);

        StatelessTransparentCommitter::<E, FRI, T> {
    ...
    fn precompute(&self, poly: &Polynomial<E, Coefficients>) -> Option<Self::IntermediateData> {
        ...
        let original_tree = < < FRI as FriIop<E> >::IopType as IOP<E::Fr> >::create(&original_poly_lde.as_ref());

        Some((original_poly_lde, original_tree))
    }

    fn commit_single(&self, poly: &Polynomial<E, Coefficients>) -> (Self::Commitment, Option<Self::IntermediateData>) {
    ...
    fn commit_multiple(&self, polynomials: Vec<&Polynomial<E, Coefficients>>, degrees: Vec<usize>, aggregation_coefficient: E::Fr) -> (Self::Commitment, Option<Vec<Self::IntermediateData>>) {
        unimplemented!()
    }
    ...
    fn open_single(
        ...
        let q_poly = Polynomial::<E, Coefficients>::from_coeffs(division_result).expect("must be small enough");
        ...
        while domain_indexes.len() < self.num_queries {
            ...
            let coset_index_values = < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Combiner as CosetCombiner<E::Fr>>::get_coset_for_natural_index(domain_idx, lde_size);
        ...
        for idx in domain_indexes.into_iter() {
            let original_value = < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Combiner as CosetCombiner<E::Fr>>::get_for_natural_index(original_poly_lde.as_ref(), idx);
    ...
    fn open_multiple(
        ...
        let mut q_poly: Option<Polynomial::<E, Coefficients>> = None;

        let mut alpha = E::Fr::one();

        for q in division_results.into_iter() {
            if let Some(q_poly) = q_poly.as_mut() {
                let q = Polynomial::<E, Coefficients>::from_coeffs(q).expect("must be small enough");
        ...
        // for idx in domain_indexes.clone().into_iter() {
        //     let value = < < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::Combiner as CosetCombiner<E::Fr>>::get_for_natural_index(q_poly_lde.as_ref(), idx);
    ...
    fn verify_single(&self, commitment: &Self::Commitment, at_point: E::Fr, claimed_value: E::Fr, proof: &Self::OpeningProof, prng: &mut Self::Prng) -> bool {
        ...
        let lde_domain = Domain::<E::Fr>::new_for_size(lde_size as u64).expect("large enough domain must exist");
        ...
        for (domain_idx, original_poly_query) in domain_indexes.clone().into_iter()
            ...
            let is_in_commitment = < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::verify_query(&original_poly_query.1, commitment);
            if !is_in_commitment {
                return false;
            }
    ...
    fn verify_multiple_openings(
        ...
        let mut simulated_q_poly_values = vec![E::Fr::zero(); domain_indexes.len()];
        ...
        for subpoly_index in 0..original_poly_queries_vec.len() {
            ...
            for (domain_idx, original_poly_query) in domain_indexes.clone().into_iter()
                ...
                let is_in_commitment = < <FRI as FriIop<E> >::IopType as IOP<E::Fr> >::verify_query(&original_poly_query.1, subpoly_commitment);
                if !is_in_commitment {
...
mod test {
    ...
    fn test_small_transparent_commitment() {
        ...
        let poly = Polynomial::<Bn256, _>::from_coeffs(coeffs).unwrap();

        let mut transcript = Blake2sTranscript::<Fr>::new();

        type Iop = TrivialBlake2sIOP<Fr>;
        type Fri = NaiveFriIop<Bn256, Iop>;
        type Committer = StatelessTransparentCommitter<Bn256, Fri, Blake2sTranscript<Fr>>;
        ...
        let committer = <Committer as CommitmentScheme<Bn256>>::new_for_size(SIZE, meta);
