...
impl<E: Engine, I: IOP<E::Fr>> NaiveFriIop<E, I> {
    pub fn verify_prototype(
        proof: &FRIProofPrototype<E, I>,
        leaf_values: &Polynomial<E, Values>,
        ...
        let domain = Domain::<E::Fr>::new_for_size(
            (proof.initial_degree_plus_one * proof.lde_factor) as u64,
        )?;
        ...
        debug_assert_eq!(E::Fr::one(), omega_inv.pow([domain.size]));

        let mut expected_value: Option<E::Fr> = None;
        let mut domain_size = domain.size as usize;
        let mut domain_idx = natural_element_index;

        for (iop_values, iop_challenge) in Some(leaf_values)
            .into_iter()
            .chain(&proof.intermediate_values)
            .zip(proof.challenges.iter())
        {
            let coset_values = <I::Combiner as CosetCombiner<E::Fr>>::get_coset_for_natural_index(
                domain_idx,
                domain_size,
            );
        ...
        let mut expected_value_from_coefficients = E::Fr::zero();
    ...
    pub fn verify_proof_queries(
        proof: &FRIProof<E::Fr, I>,
        natural_element_indexes: Vec<usize>,
        degree: usize,
        expected_values_from_oracle: &[E::Fr],
        fri_challenges: &[E::Fr],
    ) -> Result<bool, SynthesisError> {
        let mut two = E::Fr::one();
        two.double();
        ...
        for ((query, natural_element_index), expected_value_from_oracle) in proof
            .queries
            .iter()
            ...
            .zip(expected_values_from_oracle.iter())
        {
            ...
            for (round, ((root, queries), iop_challenge)) in proof
                .roots
                .iter()
                ...
                .enumerate()
            {
                ...
                for q in queries.iter() {
                    if !coset_values.contains(&q.natural_index()) {
                        println!(
                            "Coset values do not contain query index {}",
                            q.natural_index()
                        );
                        return Ok(false);
                    }
                ...
                if round == 0 {
                    for q in queries.iter() {
                        if q.natural_index() == natural_element_index
                            && q.value() != *expected_value_from_oracle
                        {
                            println!(
                                "Expected {}, got {} from query",
                ...
                for (c, q) in coset_values.iter().zip(queries.iter()) {
                    let tree_index =
                        <I::Combiner as CosetCombiner<E::Fr>>::natural_index_into_tree_index(*c);
                    if q.tree_index() != tree_index {
                        return Err(SynthesisError::PolynomialDegreeTooLarge);
                    }
                    assert!(
                        q.natural_index() == *c,
                        "coset values and produced queries are expected to be sorted!"
                    );
                }
                ...
                if let Some(value) = expected_value {
                    if !coset_values.contains(&domain_idx) {
                        println!(
                            "Coset values {:?} do not containt required index {}",
                            coset_values, domain_idx
                        );
                        return Ok(false);
                    }
                    ...
                    let q: Vec<_> = queries
                        .iter()
                        .filter(|el| el.natural_index() == domain_idx)
                        .collect();
                    if q.len() != 1 {
                        println!(
                            "Queries containt duplicate opening for required index {}",
                            domain_idx
                        );
                        return Ok(false);
                    }
                    ...
                    if supplied_value != value {
                        println!(
                            "Query value {} is not equal to the expected value {} for round {}",
                            supplied_value, value, round
                        );
                        return Ok(false);
                    }
                }
