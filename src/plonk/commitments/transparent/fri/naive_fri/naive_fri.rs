...
pub struct NaiveFriIop<E: Engine, I: IOP<E::Fr>> {
    _marker_f: std::marker::PhantomData<E::Fr>,
    _marker_i: std::marker::PhantomData<I>,
}

impl<E: Engine, I: IOP<E::Fr>> FriIop<E> for NaiveFriIop<E, I> {
    const DEGREE: usize = 2;

    type IopType = I;
    type ProofPrototype = FRIProofPrototype<E, I>;
    type Proof = FRIProof<E::Fr, I>;
    type Params = ();

    fn proof_from_lde<
        P: Prng<
            E::Fr,
            Input = <<I::Tree as IopTree<E::Fr>>::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput,
        >,
    ...
    fn verify_proof_with_challenges(
        ...
        Self::verify_proof_queries(
            proof,
            natural_element_indexes,
            Self::DEGREE,
            expected_value,
            fri_challenges,
        )
...
pub struct FRIProofPrototype<E: Engine, I: IOP<E::Fr>> {
    pub l0_commitment: I,
    pub intermediate_commitments: Vec<I>,
    pub intermediate_values: Vec<Polynomial<E, Values>>,
    pub challenges: Vec<E::Fr>,
    pub final_root: <<I::Tree as IopTree<E::Fr>>::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput,
...
impl<E: Engine, I: IOP<E::Fr>> FriProofPrototype<E::Fr, I> for FRIProofPrototype<E, I> {
    fn get_roots(
        &self,
    ) -> Vec<<<I::Tree as IopTree<E::Fr>>::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput> {
        let mut roots = vec![];
    ...
    fn get_final_root(
        &self,
    ) -> <<I::Tree as IopTree<E::Fr>>::TreeHasher as IopTreeHasher<E::Fr>>::HashOutput {
        self.final_root.clone()
    }

    fn get_final_coefficients(&self) -> Vec<E::Fr> {
...
impl<E: Engine, I: IOP<E::Fr>> NaiveFriIop<E, I> {
    ...
    >(
        ...
        worker: &Worker,
        prng: &mut P,
    ) -> Result<FRIProofPrototype<E, I>, SynthesisError> {
        ...
        let num_steps =
            log2_floor(initial_degree_plus_one / output_coeffs_at_degree_plus_one) as usize;

        let initial_polynomial = lde_values.ifft(&worker, &mut None);
        ...
        for step in 0..num_steps {
            let mut next_coefficients = vec![E::Fr::zero(); next_domain_size];
            let coeffs_slice: &[E::Fr] = coeffs.as_ref();
            assert!(next_coefficients.len() * 2 == coeffs_slice.len());

            worker.scope(next_coefficients.len(), |scope, chunk| {
                for (v, old) in next_coefficients
                    .chunks_mut(chunk)
    ...
    >(
        ...
        let omegas_inv_ref: &[E::Fr] = precomputations.omegas_inv_ref();
        ...
        let final_poly_values = Polynomial::<E, Values>::from_values(values_slice.to_vec())?;
        let final_poly_coeffs = final_poly_values.ifft(&worker, &mut None);
        ...
        assert!(
            degree < output_coeffs_at_degree_plus_one,
            "polynomial degree is too large, coeffs = {:?}",
            final_poly_coeffs
        );
        ...
        println!(
            "Done FRI for degree {} in {:?}",
            lde_values.size() / lde_factor,
            start.elapsed()
        );
