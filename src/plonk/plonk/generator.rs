...
pub fn setup_with_precomputations<E: Engine, C: Circuit<E>, CP: CTPrecomputations<E::Fr>>(
    ...
    let sigma_1_commitment_data =
        ProvingAssembly::<E>::commit_single_poly(&sigma_1, &bases, &worker)?;
    let sigma_2_commitment_data =
        ProvingAssembly::<E>::commit_single_poly(&sigma_2, &bases, &worker)?;
    let sigma_3_commitment_data =
        ProvingAssembly::<E>::commit_single_poly(&sigma_3, &bases, &worker)?;
    ...
    let q_l_lde =
        q_l.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let q_r_lde =
        q_r.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let q_o_lde =
        q_o.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let q_m_lde =
        q_m.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let q_c_lde =
        q_c.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;

    let s_id_lde =
        s_id.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;

    let sigma_1_lde =
        sigma_1.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let sigma_2_lde =
        sigma_2.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
    let sigma_3_lde =
        sigma_3.bitreversed_lde_using_bitreversed_ntt(&worker, 4, &coset_generator, &mut None)?;
