...
impl<E: Engine> ConstraintSystem<E> for ProvingAssembly<E> {
        ...
        F: FnOnce() -> Result<E::Fr, SynthesisError>,
    {
        ...
        let gate = Gate::<E::Fr>::new_enforce_constant_gate(
            input_var,
            Some(E::Fr::zero()),
            self.dummy_variable(),
        );
    ...
    fn new_gate(
        ...
        coeffs: (E::Fr, E::Fr, E::Fr, E::Fr, E::Fr),
    ...
    fn enforce_mul_2(&mut self, variables: (Variable, Variable)) -> Result<(), SynthesisError> {
        ...
        let gate = Gate::<E::Fr>::new_gate(
            (v_0, v_1, self.dummy_variable()),
...
impl<E: Engine> ProvingAssembly<E> {
    pub fn new() -> Self {
        ...
        tmp.enforce_constant(zero, E::Fr::zero())
            .expect("should have no issues");
    ...
    > {
        ...
        worker.scope(self.aux_gates.len(), |scope, chunk| {
            ...
            {
                scope.spawn(move |_| {
                    for (((((gate, q_l), q_r), q_o), q_m), q_c) in gate
                        .iter()
                        .zip(q_l.iter_mut())
                        .zip(q_r.iter_mut())
                        .zip(q_o.iter_mut())
    ...
    pub(crate) fn calculate_permutations_as_in_a_paper(
        &self,
    ...
    pub(crate) fn output_setup_polynomials(
        ...
        (
            Polynomial<E, Coefficients>, // q_l
            Polynomial<E, Coefficients>, // q_r
            Polynomial<E, Coefficients>, // q_o
            Polynomial<E, Coefficients>, // q_m
            Polynomial<E, Coefficients>, // q_c
            Polynomial<E, Coefficients>, // s_id
            Polynomial<E, Coefficients>, // sigma_1
            Polynomial<E, Coefficients>, // sigma_2
            Polynomial<E, Coefficients>, // sigma_3
        ),
        SynthesisError,
    > {
        assert!(self.is_finalized);
        ...
        let s_id = s_id.ifft(&worker, &mut None);
        let sigma_1 = sigma_1.ifft(&worker, &mut None);
        let sigma_2 = sigma_2.ifft(&worker, &mut None);
        let sigma_3 = sigma_3.ifft(&worker, &mut None);

        let q_l = q_l.ifft(&worker, &mut None);
        let q_r = q_r.ifft(&worker, &mut None);
        let q_o = q_o.ifft(&worker, &mut None);
        let q_m = q_m.ifft(&worker, &mut None);
        let q_c = q_c.ifft(&worker, &mut None);
    ...
    pub fn is_satisfied(mut self) -> bool {
        ...
        for (i, gate) in self.input_gates.iter().enumerate() {
            ...
            assert!(
                q_c.is_zero(),
                "should not hardcode a constant into the input gate"
            );

            let a_value = self
                .get_value(*gate.a_wire())
                .expect("must get a variable value");
            let b_value = self
                .get_value(*gate.b_wire())
                .expect("must get a variable value");
            let c_value = self
                .get_value(*gate.c_wire())
                .expect("must get a variable value");
    ...
    fn calculate_inverse_vanishing_polynomial_in_a_coset(
        &self,
        worker: &Worker,
        poly_size: usize,
        ...
        let mut numerator =
            Polynomial::<E, Values>::from_values(vec![multiplicative_generator; poly_size])?;
        ...
        let mut denominator = Polynomial::<E, Values>::from_values(vec![shift; poly_size])?;
    ...
    fn calculate_lagrange_poly(
        ...
        let mut poly = Polynomial::<E, Values>::from_values(vec![E::Fr::zero(); poly_size])?;

        poly.as_mut()[poly_number] = E::Fr::one();

        Ok(poly.ifft(&worker, &mut None))
    }
}
...
pub struct PlonkSetupPrecomputation<E: Engine> {
    pub q_l_aux: Polynomial<E, Values>,
    pub q_r_aux: Polynomial<E, Values>,
    pub q_o_aux: Polynomial<E, Values>,
    pub q_m_aux: Polynomial<E, Values>,
    pub q_c_aux: Polynomial<E, Values>,
    pub s_id_aux: Polynomial<E, Values>,
    pub sigma_1_aux: Polynomial<E, Values>,
    pub sigma_2_aux: Polynomial<E, Values>,
    pub sigma_3_aux: Polynomial<E, Values>,
}

struct OpeningRequest<'a, E: Engine> {
    polynomials: Vec<&'a Polynomial<E, Coefficients>>,
    opening_point: E::Fr,
...
impl<E: Engine> ProvingAssembly<E> {
    pub(crate) fn commit_single_poly(
        ...
        let bss = Arc::new(bases.to_vec());
        let exps = Arc::new(reprs);
        let result = dense_multiexp(&worker, bss, exps, &mut None);
    ...
    >(
        ...
        let w_l = Polynomial::<E, Values>::from_values_unpadded(w_l)?;
        let w_r = Polynomial::<E, Values>::from_values_unpadded(w_r)?;
        let w_o = Polynomial::<E, Values>::from_values_unpadded(w_o)?;
        ...
        let z_1 = {
            ...
            let s_id_1 = Polynomial::<E, Values>::from_values_unpadded(s_id_1)?;
            ...
            let s_id_2 = Polynomial::<E, Values>::from_values_unpadded(s_id_2)?;
            ...
            let s_id_3 = Polynomial::<E, Values>::from_values_unpadded(s_id_3)?;
            ...
            Polynomial::<E, Values>::from_values(prepadded)?
        };
        ...
        let z_2 = {
            ...
            let sigma_1 = Polynomial::<E, Values>::from_values_unpadded(sigma_1)?;
            let mut w_l_contribution = w_l_plus_gamma.clone();
            w_l_contribution.add_assign_scaled(&worker, &sigma_1, &beta);
            drop(sigma_1);

            let sigma_2 = convert_to_field_elements(&sigma_2, &worker);
            let sigma_2 = Polynomial::<E, Values>::from_values_unpadded(sigma_2)?;
            let mut w_r_contribution = w_r_plus_gamma.clone();
            ...
            let sigma_3 = Polynomial::<E, Values>::from_values_unpadded(sigma_3)?;
            let mut w_o_contribution = w_o_plus_gamma.clone();
            ...
            let z_2 = Polynomial::<E, Values>::from_values(prepadded)?;

            z_2
        };

        assert!(
            z_2.as_ref().last().expect("must exist") == z_1.as_ref().last().expect("must exist")
        );
        ...
        let (q_l, q_r, q_o, q_m, q_c, s_id, sigma_1, sigma_2, sigma_3) =
            self.output_setup_polynomials(&worker)?;
        ...
        let mut vanishing_poly_inverse_bitreversed = self
            .calculate_inverse_vanishing_polynomial_in_a_coset(
                &worker,
                q_l_coset_lde_bitreversed.size(),
        ...
        fn get_degree<E: Engine>(poly: &Polynomial<E, Coefficients>) -> usize {
        ...
        let z_1_shifted_coset_lde_bitreversed =
            z_1_shifted.clone().bitreversed_lde_using_bitreversed_ntt(
                &worker,
                4,
                &E::Fr::multiplicative_generator(),
                &mut None,
        ...
        let z_2_shifted_coset_lde_bitreversed =
            z_2_shifted.clone().bitreversed_lde_using_bitreversed_ntt(
                &worker,
                4,
                &E::Fr::multiplicative_generator(),
                &mut None,
        ...
        let l_0 =
            self.calculate_lagrange_poly(&worker, required_domain_size.next_power_of_two(), 0)?;
        let l_n_minus_one =
            self.calculate_lagrange_poly(&worker, required_domain_size.next_power_of_two(), n - 1)?;
        ...
        let t_poly =
            t_1.icoset_fft_for_generator(&worker, &E::Fr::multiplicative_generator(), &mut None);

        debug_assert!(get_degree(&t_poly) <= 3 * n);
        ...
        let mut inverse_vanishing_at_z =
            self.evaluate_inverse_vanishing_poly(required_domain_size.next_power_of_two(), z);
...
mod test {
    ...
    fn test_bench_plonk_bls12() {
        ...
        let bases = {
            ...
            let mut powers_of_tau = Polynomial::<Eng, _>::from_coeffs(powers_of_tau).unwrap();
        ...
        for size in sizes.into_iter() {
            ...
            let _ = prover
                .prove_with_setup_precomputed::<_, _, Transcr>(
                    &setup_precomp,
                    &worker,
                    &omegas_bitreversed,
