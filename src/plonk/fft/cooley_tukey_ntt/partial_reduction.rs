...
pub(crate) fn best_ct_ntt_partial_reduction<
    F: PartialTwoBitReductionField,
    P: CTPrecomputations<F>,
>(
    a: &mut [F],
    worker: &Worker,
    log_n: u32,
    ...
    let log_cpus = if let Some(hint) = use_cpus_hint {
        assert!(hint <= worker.get_num_cpus());
...
pub(crate) fn serial_ct_ntt_partial_reduction<
    F: PartialTwoBitReductionField,
    P: CTPrecomputations<F>,
>(
    a: &mut [F],
    ...
    assert_eq!(
        a.len(),
        precomputed_omegas.domain_size(),
        "precomputation size is invalid for ntt"
    );
    ...
    while num_groups < half_n {
        ...
        for k in 0..num_groups {
            ...
            for j in idx_1..idx_2 {
                ...
                debug_assert!(
                    tmp_u.overflow_factor() < 4,
                    "factor is {} for num groups {}",
                    tmp_u.overflow_factor(),
                    num_groups
                );
                debug_assert!(
                    tmp_v.overflow_factor() < 4,
                    "factor is {} for num groups {}",
                    tmp_v.overflow_factor(),
                    num_groups
                );
    ...
    if num_groups < n {
        ...
        for k in 0..num_groups {
            ...
            for j in idx_1..idx_2 {
                ...
                debug_assert!(
                    tmp_u.overflow_factor() < 1,
                    "factor is {} for num groups {}",
                    tmp_u.overflow_factor(),
                    num_groups
                );
                debug_assert!(
                    tmp_v.overflow_factor() < 1,
                    "factor is {} for num groups {}",
                    tmp_v.overflow_factor(),
                    num_groups
                );
...
pub(crate) fn parallel_ct_ntt_partial_reduction<
    F: PartialTwoBitReductionField,
    P: CTPrecomputations<F>,
>(
    a: &mut [F],
            ...
            scope.spawn(move |_| {
                ...
                while num_groups < half_n {
                    if num_groups >= to_spawn {
                        ...
                        let chunk =
                            Worker::chunk_size_for_num_spawned_threads(num_groups, to_spawn);
                        ...
                        for k in 0..num_groups {
                            ...
                            let chunk =
                                Worker::chunk_size_for_num_spawned_threads(group_size, to_spawn);
    ...
    fn test_bench_ct_serial_fft() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            if let Some(diff) = diff_pr {
                println!(
                    "Partial reduction: speed up is {}%.",
                    diff.as_nanos() * 100 / ntt_time.as_nanos()
                );
            } else {
    ...
    fn test_bench_ct_parallel_fft() {
        ...
        for poly_size in poly_sizes.into_iter() {
            ...
            let (res3, elapsed3) = {
                ...
                let mut coeffs = (0..poly_size).map(|_| Fr::rand(rng)).collect::<Vec<_>>();
                let start = Instant::now();
                parallel_ct_ntt_partial_reduction(
                    &mut coeffs,
                    &worker,
                    log_n,
                    worker.log_num_cpus(),
                ...
                let finish = start.elapsed();
                println!(
                    "parallel NTT with partial reduction for size {} taken {:?}",
                    poly_size, finish
                );
