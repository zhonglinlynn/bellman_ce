...
impl<E: Engine> ConstraintSystem<E> for ProvingAssembly<E> {
        ...
        F: FnOnce() -> Result<E::Fr, SynthesisError>,
    {
        ...
        let gate = Gate::<E>::new_enforce_constant_gate(
            input_var,
            Some(E::Fr::zero()),
            self.dummy_variable(),
        );
    ...
    fn enforce_constant(
        ...
        let gate =
            Gate::<E>::new_enforce_constant_gate(variable, Some(constant), self.dummy_variable());
        self.aux_gates.push(gate);
        self.n += 1;
...
impl<E: Engine> ProvingAssembly<E> {
    ...
    pub(crate) fn new() -> Self {
        ...
        let one = tmp
            .alloc(|| Ok(E::Fr::one()))
            .expect("should have no issues");
        tmp.enforce_constant(one, E::Fr::one())
            .expect("should have no issues");
...
pub fn prove_nonhomomorphic<
    E: Engine,
    S: CommitmentScheme<E::Fr, Prng = T>,
    T: Transcript<E::Fr, Input = S::Commitment>,
    C: Circuit<E>,
>(
    circuit: &C,
    ...
    let (q_l, q_r, q_o, q_m, q_c, s_id, sigma_1, sigma_2, sigma_3) =
        assembly.output_setup_polynomials(&worker)?;
    ...
    let mut vanishing_poly_inverse = assembly.calculate_inverse_vanishing_polynomial_in_a_coset(
        &worker,
        q_c_lde.size(),
    ...
    let l_0 =
        assembly.calculate_lagrange_poly(&worker, required_domain_size.next_power_of_two(), 0)?;
    let l_n_minus_one = assembly.calculate_lagrange_poly(
        &worker,
        required_domain_size.next_power_of_two(),
        n - 1,
    )?;
    ...
    let mut inverse_vanishing_at_z =
        assembly.evaluate_inverse_vanishing_poly(required_domain_size.next_power_of_two(), z);
    ...
    let opening_polynomials = vec![
        &a_poly, &b_poly, &c_poly, &q_l, &q_r, &q_o, &q_m, &q_c, &s_id, &sigma_1, &sigma_2,
        &sigma_3, &z_1, &z_2, &z_1, &z_2,
    ];
    ...
    let opening_points = vec![
        z, z, z, z, z, z, z, z, z, z, z, z, z, z, z_by_omega, z_by_omega,
    ];
    ...
    let t_opening_proof =
        large_committer.open_single(&t_poly, z, t_at_z, &t_aux.as_ref(), &mut transcript);
...
pub fn prove_nonhomomorphic_chunked<
    E: Engine,
    S: CommitmentScheme<E::Fr, Prng = T>,
    T: Transcript<E::Fr, Input = S::Commitment>,
    C: Circuit<E>,
>(
    ...
    let opening_points = vec![
        z, z, z, z, z, z, z, z, z, z, z, z, z, z, z_by_omega, z_by_omega, z, z, z,
    ];
