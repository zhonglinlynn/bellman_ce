...
use crate::{gpu::LockedMultiFFTKernel, SynthesisError};
...
pub trait GateInternal<E: Engine>: Send + Sync + 'static + std::any::Any + std::fmt::Debug {
    ...
    fn verify_on_row<'a>(
        &self,
        row: usize,
        poly_storage: &AssembledPolynomialStorage<'a, E>,
        last_row: bool,
    ) -> E::Fr;
...
pub trait Gate<E: Engine>:
    GateInternal<E> + Sized + Clone + std::hash::Hash + std::default::Default
{
...
impl<E: Engine> MainGate<E> for Width4MainGateWithDNext {
    ...
    fn contribute_into_quotient_for_public_inputs<'a, 'b>(
        ...
        let mut inputs_poly = Polynomial::<E, Values>::new_for_size(domain_size, worker)?;
        for (idx, &input) in public_inputs.iter().enumerate() {
            inputs_poly.as_mut()[idx] = input;
        }
        // go into monomial form

        let mut inputs_poly = inputs_poly.ifft(&worker, fft_kern);
        ...
        for i in 0..lde_factor {
            t_1.bitreverse_enumeration_partial(&worker, i * domain_size, domain_size);
        }
        ...
        let mut tmp = q_a_ref.fast_clone(worker);
        ...
        tmp.reuse_allocation_parallel(worker, q_b_ref);
        ...
        tmp.reuse_allocation_parallel(worker, q_c_ref);
        ...
        tmp.reuse_allocation_parallel(worker, q_d_ref);
        ...
        tmp.reuse_allocation_parallel(worker, q_m_ref);
        tmp.mul_assign(&worker, a_ref);
        tmp.mul_assign(&worker, b_ref);
        t_1.add_assign(&worker, &tmp);
        drop(q_m_ref);
        ...
        tmp.reuse_allocation_parallel(worker, q_d_next_ref);
    ...
    fn contribute_into_linearization_for_public_inputs(
        ...
        let mut result = monomials_storage
            .get_poly(PolyIdentifier::GateSetupPolynomial(name, 0))
            .clone();
    ...
    fn contribute_into_linearization_commitment_for_public_inputs<'a>(
        ...
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 0))
            .ok_or(SynthesisError::AssignmentMissing)?;
        ...
        // Q_b * B
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 1))
            .ok_or(SynthesisError::AssignmentMissing)?;
        ...
        // Q_c * C
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 2))
            .ok_or(SynthesisError::AssignmentMissing)?;
        ...
        tmp.mul_assign(&b_value);
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 4))
            .ok_or(SynthesisError::AssignmentMissing)?;
        ...
        // Q_const
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 5))
            .ok_or(SynthesisError::AssignmentMissing)?;
        aggregate.add_assign_mixed(&commitment);

        // Q_dNext * D_next
        let commitment = commitments_storage
            .get(&PolyIdentifier::GateSetupPolynomial(name, 6))
            .ok_or(SynthesisError::AssignmentMissing)?;
...
pub fn ensure_in_map_or_create<'a, 'b, E: Engine>(
    ...
    if !contains_in_scratch_or_maps {
        ...
        let rotated = if let Some(lde) = lde_without_dilation.as_ref() {
            println!(
                "dilation_value * lde_factor = {:?}*{:?}",
                dilation_value, lde_factor
            );
            let rotation_factor = dilation_value * lde_factor;
            let f = lde
                .as_ref()
                .clone_shifted_assuming_bitreversed(rotation_factor, worker)?;
            drop(lde);
        ...
        if !done {
            ...
            let domain_size = monomial.size();
            for i in 0..lde_factor {
                lde.bitreverse_enumeration_partial(&worker, i * domain_size, domain_size);
            }
...
pub trait ConstraintSystem<E: Engine> {
    ...
    fn allocate_variables_without_gate(
        &mut self,
        variables_assignments: &[Variable],
        witness_assignments: &[E::Fr],
    ) -> Result<(), SynthesisError>;
    ...
    fn add_table(
        &mut self,
        table: LookupTableApplication<E>,
    ...
    fn get_multitable(
        &self,
        functional_name: &str,
    ) -> Result<Arc<MultiTableApplication<E>>, SynthesisError>;

    fn apply_single_lookup_gate(
        &mut self,
        variables: &[Variable],
        gate: Arc<LookupTableApplication<E>>,
    ) -> Result<(), SynthesisError>;
    fn apply_multi_lookup_gate(
        &mut self,
        variables: &[Variable],
        gate: Arc<MultiTableApplication<E>>,
    ) -> Result<(), SynthesisError>;
...
impl<E: Engine> PolynomialStorage<E> {
    ...
    pub fn get_value(
        ...
        match poly {
            ...
            PolynomialInConstraint(
                PolyIdentifier::GateSetupPolynomial(gate_descr, idx),
    ...
    pub fn get_variable(
        ...
        match poly {
            PolynomialInConstraint(
                PolyIdentifier::VariablesPolynomial(idx),
                TimeDilation(dilation),
            ) => {
                let final_index = n + dilation;
...
#[derive(Clone)]
pub struct GateDensityStorage<E: Engine>(
    pub std::collections::HashMap<Box<dyn GateInternal<E>>, BitVec>,
);
...
pub struct GateConstantCoefficientsStorage<E: Engine>(
    pub std::collections::HashMap<Box<dyn GateInternal<E>>, Vec<E::Fr>>,
);
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>, MG: MainGate<E>, S: SynthesisMode>
    ConstraintSystem<E> for Assembly<E, P, MG, S>
{
    type Params = P;
    type MainGate = MG;
        ...
        F: FnOnce() -> Result<E::Fr, SynthesisError>,
    {
        ...
        let (variables_assignments, coefficients_assignments) =
            MG::format_term(main_gate, dummy).expect("must make empty padding gate");
    ...
    fn new_gate_in_batch<G: Gate<E>>(
        ...
        if S::PRODUCE_SETUP {
            if let Some(tracker) = self
                .aux_gate_density
                .0
                .get_mut(gate.as_internal() as &dyn GateInternal<E>)
            {
                if tracker.len() != n {
                ...
                self.aux_gate_density
                    .0
                    .insert(gate.clone().into_internal(), BitVec::new());
                let tracker = self
                    .aux_gate_density
                    .0
                    .get_mut(gate.as_internal() as &dyn GateInternal<E>)
    ...
    fn allocate_variables_without_gate(
        ...
        let n = self
            .trace_step_for_batch
            .expect("may only be called in a batch");
        // make zero-enumerated index
        let n = n - 1;
    ...
    fn add_table(
        ...
        assert!(
            table.applies_over().len() == 3,
            "only support tables of width 3"
        );
        assert!(
            table.can_be_combined(),
            "can only add tables that are combinable"
        );
        ...
        self.individual_table_entries
            .insert(table_name.clone(), vec![]);
    ...
    fn add_multitable(&mut self, table: MultiTableApplication<E>) -> Result<(), SynthesisError> {
        ...
        self.multitable_selectors
            .insert(table_name.clone(), BitVec::new());
        self.individual_table_entries
            .insert(table_name.clone(), vec![]);
    ...
    fn apply_single_lookup_gate(
        ...
        let n = self
            .trace_step_for_batch
            .expect("may only add table constraint in a transaction");
        // make zero-enumerated index
        let n = n - 1;
    ...
    fn apply_multi_lookup_gate(
        &mut self,
        variables: &[Variable],
        table: Arc<MultiTableApplication<E>>,
    ) -> Result<(), SynthesisError> {
        let n = self
            .trace_step_for_batch
            .expect("may only add table constraint in a transaction");
    ...
    Assembly<E, P, MG, S>
{
    fn allocate_into_storage<G: Gate<E>>(
        ...
        if S::PRODUCE_SETUP {
            ...
            debug_assert!(
                coeffs_it.next().is_none(),
                "must consume all the coefficients for gate"
            );
        }
        ...
        debug_assert!(
            variable_it.next().is_none(),
            "must consume all variables for gate"
        );
    ...
    fn add_gate_into_list<G: Gate<E>>(&mut self, gate: &G) {
        if !self
            .gates
            .contains(gate.as_internal() as &dyn GateInternal<E>)
        {
    ...
    pub fn finalize(&mut self) {
        ...
        let new_size_candidates = [
            (self.n() + 1).next_power_of_two() - 1,
            (min_space_for_lookups + 1).next_power_of_two() - 1,
        ];
        ...
        let empty_witness =
            vec![E::Fr::zero(); <Self as ConstraintSystem<E>>::Params::WITNESS_WIDTH];
        ...
        for _ in self.n()..new_size {
            ...
            self.allocate_variables_without_gate(&empty_vars, &empty_witness)
                .unwrap();
    ...
    pub(crate) fn make_permutations(
        &self,
        worker: &Worker,
    ) -> Result<Vec<Polynomial<E, Values>>, SynthesisError> {
        ...
        for gate_idx in 0..num_gates {
            ...
            for (state_poly_index, poly_id) in poly_ids.iter().enumerate() {
                let variables_vec_ref = storage
                    .state_map
                    .get(&poly_id)
                    .expect("must get a variables polynomial");
        ...
        let mut sigmas = vec![];
        for i in 0..P::STATE_WIDTH {
            let mut cloned_domain_elements = Vec::with_capacity(domain.size as usize);
            unsafe {
                cloned_domain_elements.set_len(domain.size as usize);
            }
            fast_clone(&domain_elements, &mut cloned_domain_elements, worker);
            let mut sigma_i = Polynomial::from_values_unpadded(cloned_domain_elements).unwrap();
    ...
    fn make_setup_polynomials(
        ...
        worker: &Worker,
    ) -> Result<std::collections::HashMap<PolyIdentifier, Polynomial<E, Values>>, SynthesisError>
    {
        ...
        for &id in setup_poly_ids.into_iter() {
            ...
            let mut assembled_poly = Vec::with_capacity(total_num_gates);
            unsafe { assembled_poly.set_len(total_num_gates) };

            if num_input_gates != 0 {
                let input_gates_coeffs = &mut assembled_poly[..num_input_gates];
                input_gates_coeffs
                    .copy_from_slice(&self.inputs_storage.setup_map.get(&id).unwrap()[..]);
            }

            {
                let src = &self.aux_storage.setup_map.get(&id).unwrap()[..];
                let src_len = src.len();
                let aux_gates_coeffs =
                    &mut assembled_poly[num_input_gates..(num_input_gates + src_len)];
                // aux_gates_coeffs.copy_from_slice(src);
                fast_clone(&src, aux_gates_coeffs, worker);
            }
    ...
    pub fn create_setup<C: Circuit<E>>(
        ...
        {
            assert!(
                &<Self as ConstraintSystem<E>>::MainGate::default().into_internal()
                    == &claimed_gates_list[0]
            );
        ...
        let (mut setup_polys_values_map, mut permutation_polys) = self.perform_setup(&worker)?;

        let mut fft_kern = Some(LockedMultiFFTKernel::<E>::new(0, false));
        println!("known_gates_list len = {:?}", known_gates_list.len());
        for gate in known_gates_list.iter() {
            let setup_polys = gate.setup_polynomials();
            for id in setup_polys.chunks_exact(2) {
                let values0 = setup_polys_values_map
                    .remove(&id[0])
                    .expect("must contain setup poly")
                    .clone_padded_to_domain(worker)?;
                let mut values1 = setup_polys_values_map
                    .remove(&id[1])
                    .expect("must contain setup poly")
                    .clone_padded_to_domain(worker)?;
                let (mon0, mon1) = values0.ifft_2(values1, &worker, &mut fft_kern);

                setup.gate_setup_monomials.push(mon0);
                setup.gate_setup_monomials.push(mon1);
            }

            for id in setup_polys.chunks_exact(2).remainder() {
                let mut values = setup_polys_values_map
                    .remove(id)
                    .expect("must contain setup poly")
                    .clone_padded_to_domain(worker)?;

                let mon = values.ifft(&worker, &mut fft_kern);
                setup.gate_setup_monomials.push(mon);
            }
        }

        println!("permutation_polys len = {:?}", permutation_polys.len());
        assert_eq!(permutation_polys.len() % 2, 0);
        for perm in permutation_polys.chunks(2) {
            let mut perm0 = perm[0].fast_clone(worker);
            let mut perm1 = perm[1].fast_clone(worker);
            let (mon0, mon1) = perm0.ifft_2(perm1, &worker, &mut fft_kern);

            setup.permutation_monomials.push(mon0);
            setup.permutation_monomials.push(mon1);
        }

        let mut gate_selector_values = self.output_gate_selectors(&worker)?;

        if known_gates_list.len() > 1 {
            assert_eq!(
                gate_selector_values.len(),
                known_gates_list.len(),
        ...
        {
            assert_eq!(gate_selector_values.len(), 2);
            let mut iter = gate_selector_values.into_iter();
            let poly0 = Polynomial::from_values(iter.next().unwrap())?;
            let poly1 = Polynomial::from_values(iter.next().unwrap())?;

            let (mon0, mon1) = poly0.ifft_2(poly1, worker, &mut fft_kern);
            setup.gate_selectors_monomials.push(mon0);
            setup.gate_selectors_monomials.push(mon1);
        }

        println!("num_table_lookups  = {:?}", self.num_table_lookups);
        if self.tables.len() > 0 && self.num_table_lookups > 0 {
            ...
            for tail in table_tails.into_iter() {
                ...
                let mon = poly.ifft(&worker, &mut fft_kern);
            ...
            let table_type_values = self.calculate_table_type_values(worker)?;
    ...
    pub fn perform_setup(
        ...
        (
            std::collections::HashMap<PolyIdentifier, Polynomial<E, Values>>,
            Vec<Polynomial<E, Values>>,
        ),
        SynthesisError,
    > {
        let map = self.make_setup_polynomials(true, worker)?;
        let permutation_polys = self.make_permutations(&worker)?;

        Ok((map, permutation_polys))
    }

    pub fn output_gate_selectors(
        &self,
        worker: &Worker,
        ...
        worker.scope(poly_values.len(), |scope, chunk| {
            for (i, lh) in poly_values.chunks_mut(chunk).enumerate() {
                scope.spawn(move |_| {
    ...
    pub fn calculate_t_polynomial_values_for_single_application_tables(
        &self,
    ) -> Result<Vec<Vec<E::Fr>>, SynthesisError> {
        if !S::PRODUCE_SETUP {
            return Err(SynthesisError::AssignmentMissing);
        ...
        assert_eq!(
            width, 3,
            "only support tables that span over 3 polynomials for now"
        );
        ...
        for table in self.tables.iter() {
            ...
            println!("table.entries length = {:?}", entries[0].len());
            ...
            column_contributions
                .last_mut()
                .unwrap()
                .resize(pad_to_len, table_id);
        }

        Ok(column_contributions)
    }

    pub fn calculate_interleaved_t_polys(
        &self,
        worker: &Worker,
    ) -> Result<Vec<Vec<E::Fr>>, SynthesisError> {
        assert!(self.is_finalized);
    ...
    pub fn calculate_s_poly_contributions_from_witness(
        &self,
    ) -> Result<Vec<Vec<E::Fr>>, SynthesisError> {
        if self.tables.len() == 0 {
            return Ok(vec![]);
    ...
    pub fn calculate_table_type_values(
        ...
        // let mut values = vec![E::Fr::zero(); size];
        let mut values = fast_initialize_to_element(size, E::Fr::zero(), worker);
    ...
    pub fn calculate_lookup_selector_values(&self) -> Result<Vec<E::Fr>, SynthesisError> {
        assert!(self.is_finalized);
        ...
        // total number of gates, Input + Aux
        let size = self.n();
        println!("calculate_lookup_selector_values n = {:?}", size);
        ...
        let lookup_selector_values_t = Instant::now();
        for single_application in self.tables.iter() {
            let table_name = single_application.functional_name();
        ...
        println!(
            "lookup_selector_values setup taken {:?}",
            lookup_selector_values_t.elapsed()
        );
    ...
    pub fn calculate_masked_lookup_entries(
        ...
        // total number of gates, Input + Aux
        let size = self.n();
        print!("calculate_masked_lookup_entries size = {:?}", size);
        ...
        for _ in 0..3 {
            let mut contribution = Vec::with_capacity(size);
            unsafe {
                contribution.set_len(size);
            }
            contributions_per_column.push(contribution);
        }
        for single_application in self.tables.iter() {
            let table_name = single_application.functional_name();
    ...
    pub fn calculate_masked_lookup_entries_using_selector<'a>(
        &self,
        storage: &AssembledPolynomialStorage<E>,
        selector: &PolynomialProxy<'a, E, Values>,
    ) -> Result<Vec<Vec<E::Fr>>, SynthesisError> {
        assert!(self.is_finalized);
        if self.tables.len() == 0 {
            return Ok(vec![]);
    ...
    fn sort_by_t(
        ...
        let mut index_lookups_for_sorting =
            vec![std::collections::HashMap::with_capacity(entries_len); witness_entries.len()];
        ...
        for (idx, witness_column) in witness_entries.iter().enumerate() {
            ...
            for (witness_index, witness_value) in witness_column.iter().enumerate() {
                let reverse_lookup_index =
                    index_lookups_for_sorting[idx].get(witness_value).unwrap();
    ...
    pub fn make_state_and_witness_polynomials(
        ...
        worker.scope(full_assignments.len(), |scope, chunk| {
            for (i, lh) in full_assignments.chunks_mut(chunk).enumerate() {
                scope.spawn(move |_| {
                    // we take `values_per_leaf` values from each of the polynomial
                    // and push them into the conbinations
                    let base_idx = i * chunk;
    ...
    pub fn make_assembled_poly_storage<'a>(
        ...
        let (state_polys, witness_polys) =
            self.make_state_and_witness_polynomials(&worker, with_finalization)?;
        ...
        if S::PRODUCE_SETUP {
            let setup_polys_map = self.make_setup_polynomials(with_finalization, worker)?;
    ...
    pub fn create_monomial_storage<'a, 'b>(
        ...
        assert_eq!(value_form_storage.is_bitreversed, false);
        ...
        fn ifft_then_insert<E: Engine>(
            worker: &Worker,
            kern: &mut Option<LockedMultiFFTKernel<E>>,
            value_form_storage: &AssembledPolynomialStorage<E>,
            monomial_storage: &mut AssembledPolynomialStorageForMonomialForms<E>,
            map_type: MapType,
        ) -> Result<(), SynthesisError> {
            let mut polys = vec![];
            let mut keys = vec![];
            let (value_form_storage_map, monomial_storage_map) = match map_type {
                MapType::StateMap => (
                    &value_form_storage.state_map,
                    &mut monomial_storage.state_map,
                ),
                MapType::WitnessMap => (
                    &value_form_storage.witness_map,
                    &mut monomial_storage.witness_map,
                ),
                MapType::GateSelectors => (
                    &value_form_storage.gate_selectors,
                    &mut monomial_storage.gate_selectors,
                ),
                MapType::SetupMap => (
                    &value_form_storage.setup_map,
                    &mut monomial_storage.setup_map,
                ),
            };

            for (k, v) in value_form_storage_map.iter() {
                polys.push(v.as_ref().clone_padded_to_domain(worker)?);
                keys.push(*k);
            }
            let mon_forms = ifft_multiple(polys, worker, kern);
            for (k, p) in keys.into_iter().zip(mon_forms.into_iter()) {
                let mon_form = PolynomialProxy::from_owned(p);
                monomial_storage_map.insert(k, mon_form);
            }
    ...
    pub fn prover_stub(self, worker: &Worker) -> Result<(), SynthesisError> {
        ...
        let monomials_storage =
            Self::create_monomial_storage(&worker, &values_storage, true, &mut None)?;
        ...
        let mut lookup_data: Option<lookup_tables::LookupDataHolder<E>> = if self.tables.len() > 0 {
            ...
            let mut table_contributions_values =
            ...
            let selector_poly =
                Polynomial::<E, Values>::from_values(selector_for_lookup_values)?
                    .ifft_using_bitreversed_ntt(&worker, &omegas_inv_bitreversed, &E::Fr::one())?;

            let data = lookup_tables::LookupDataHolder::<E> {
                eta,
                f_poly_unpadded_values: Some(Polynomial::from_values_unpadded(
                    f_poly_values_aggregated,
                )?),
                t_poly_unpadded_values: Some(Polynomial::from_values_unpadded(full_t_poly_values)?),
                t_shifted_unpadded_values: Some(Polynomial::from_values_unpadded(
                    full_t_poly_values_shifted,
                )?),
                s_poly_unpadded_values: Some(Polynomial::from_values_unpadded(full_s_poly_values)?),
                s_shifted_unpadded_values: Some(Polynomial::from_values_unpadded(
                    full_s_poly_values_shifted,
                )?),
        ...
        let lookup_z_poly_in_monomial_form = if let Some(data) = lookup_data.as_mut() {
            ...
            let t_poly_monomial = t_poly_unpadded_values
                .clone_padded_to_domain(worker)?
                .ifft_using_bitreversed_ntt(&worker, &omegas_inv_bitreversed, &E::Fr::one())?;

            let s_poly_monomial = s_poly_unpadded_values
                .clone_padded_to_domain(worker)?
                .ifft_using_bitreversed_ntt(&worker, &omegas_inv_bitreversed, &E::Fr::one())?;
        ...
        if let Some(z_poly_in_monomial_form) = lookup_z_poly_in_monomial_form.as_ref() {
            ...
            let mut z_lde = z_poly_in_monomial_form
                .clone()
                .bitreversed_lde_using_bitreversed_ntt(
                    &worker,
                    lde_factor,
            ...
            let domain_size = z_poly_in_monomial_form.size();
            for i in 0..lde_factor {
                z_lde.bitreverse_enumeration_partial(&worker, i * domain_size, domain_size);
            }
            ...
            let domain_size = data.s_poly_monomial.as_ref().unwrap().size();
            for i in 0..lde_factor {
                s_lde.bitreverse_enumeration_partial(&worker, i * domain_size, domain_size);
            }
            ...
            let domain_size = data.t_poly_monomial.as_ref().unwrap().size();
            for i in 0..lde_factor {
                t_lde.bitreverse_enumeration_partial(&worker, i * domain_size, domain_size);
            }
            ...
            let f_lde = {
                ...
                let mut table_type_lde = lookup_data
                    .as_ref()
                    .unwrap()
                ...
                let domain_size = lookup_data
                    ...
                    .size();
                for i in 0..lde_factor {
                    table_type_lde.bitreverse_enumeration_partial(
                        &worker,
                        i * domain_size,
                        domain_size,
                    );
                ...
                let mut lookup_selector_lde = lookup_data
                    .as_ref()
                    .unwrap()
                ...
                let domain_size = lookup_data
                    ...
                    .size();
                for i in 0..lde_factor {
                    lookup_selector_lde.bitreverse_enumeration_partial(
                        &worker,
                        i * domain_size,
                        domain_size,
                    );
            ...
            let domain_size = l_last.size();
            let mut l_last_coset_lde_bitreversed = l_last.bitreversed_lde_using_bitreversed_ntt(
                &worker,
                lde_factor,
                &coset_factor,
                &mut None,
            ...
            for i in 0..lde_factor {
                l_last_coset_lde_bitreversed.bitreverse_enumeration_partial(
                    &worker,
                    i * domain_size,
                    domain_size,
                );
        ...
        let t_poly = t_poly.icoset_fft_for_generator(&worker, &coset_factor, &mut None);
        ...
        println!("Quotient poly degree = {}", get_degree::<E>(&t_poly));
        ...
        for p in sorted_opening_requests.into_iter() {
            ...
            let entry = query_values_map_over_dilation
                .entry(dilation_value)
                .or_insert(vec![]);
            entry.push(value);
...
mod test {
    ...
    impl<E: Engine> Circuit<E> for TestCircuit4WithLookups<E> {
        ...
        fn synthesize<CS: ConstraintSystem<E>>(&self, cs: &mut CS) -> Result<(), SynthesisError> {
            ...
            let range_table =
                LookupTableApplication::new_range_table_of_width_3(2, columns.clone())?;
    ...
    fn test_circuit_setup_and_prove() {
        ...
        let setup = assembly
            .create_setup::<TestCircuit4WithLookups<Bn256>>(&worker)
            .unwrap();
        ...
        let proof = assembly
            .create_proof::<TestCircuit4WithLookups<Bn256>, RollingKeccakTranscript<Fr>>(
                &worker, &setup, &crs_mons, None,
            )
            .unwrap();
