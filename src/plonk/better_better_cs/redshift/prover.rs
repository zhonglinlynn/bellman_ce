...
impl<E: Engine, H: BinaryTreeHasher<E::Fr>> RedshiftProver<E, H> {
    ...
    pub(crate) fn second_step_from_first_step(
        ...
        let mut grand_products_protos_with_gamma =
            first_state.witness_polys_unpadded_values.clone();
        ...
        let grand_product_in_monomial_form =
            z.ifft_using_bitreversed_ntt(&worker, &self.precomputed_omegas_inv, &E::Fr::one())?;
        ...
        for mut p in first_state.witness_polys_unpadded_values.into_iter() {
            p.pad_to_domain()?;
            let p =
                p.ifft_using_bitreversed_ntt(&worker, &self.precomputed_omegas_inv, &E::Fr::one())?;
    ...
    pub(crate) fn third_step_from_second_step(
        ...
        assert!(
            max_degree <= 5,
            "we do not support very high order constraints yet"
        );
        ...
        let mut t = Polynomial::<E, Values>::new_for_size(quotient_degree, worker)?;
        ...
        for (i, (gate, constants)) in self
            .sorted_gates
            .iter()
            .zip(self.gate_constants.iter())
            .enumerate()
        {
                    ...
                    for poly in term.1.iter() {
                        match poly {
                            PolynomialInConstraint::VariablesPolynomial(
                                ...
                                let poly_lde_part = poly_lde_ref
                            ...
                            PolynomialInConstraint::VariablesPolynomial(
                                ...
                                let dilated_poly = if let Some(dilated_poly) =
                                    extra_precomputations_storage_by_time_dilation
                                        .get(&(*poly_num, TimeDilation(*dilation)))
                                {
                                    dilated_poly
                                } else {
                                    let poly_lde_ref = &second_state.witness_polys_ldes[*poly_num];
                                    ...
                                    let dilated_poly = poly_lde_part
                                        .clone_shifted_assuming_bitreversed(shift, &worker)?;

                                    extra_precomputations_storage_by_time_dilation
                                        .insert((*poly_num, TimeDilation(*dilation)), dilated_poly);
                            ...
                            PolynomialInConstraint::SetupPolynomial(
                                ...
                                let process_public_inputs = if constant_term_index.is_some() {
                                    public_inputs_processed == false
                                        && constant_term_index.unwrap() == *poly_num
                                } else {
                                    false
                                };
                                ...
                                if process_public_inputs {
                                    ...
                                    let mut inputs_poly = Polynomial::<E, Values>::new_for_size(
                                        required_domain_size,
                                        worker,
                                    )?;

                                    for (idx, &input) in
                                        second_state.input_values.iter().enumerate()
                                    {
                                    ...
                                    let mut inputs_poly = inputs_poly
                                        .bitreversed_lde_using_bitreversed_ntt(
                                            &worker,
            ...
            if use_gate_selectors {
                let selector_idx = (&mut selectors_range_it)
                    .next()
                    .expect(&format!("must get gate selector for gate {}", i));
                let poly_lde_ref = &setup.polynomial_ldes[selector_idx];
                let poly_lde_part =
                    poly_lde_ref.clone_subset_assuming_bitreversed(partition_factor)?;
                global_scratch_space.mul_assign(&worker, &poly_lde_part);
                t.add_assign(&worker, &global_scratch_space);
            }
        }
        ...
        let grand_product_lde_bitreversed = second_state.grand_product_polynomial_lde[0]
            .clone_subset_assuming_bitreversed(partition_factor)?;

        let shift = quotient_degree_factor;

        let grand_product_shifted_lde_bitreversed =
            grand_product_lde_bitreversed.clone_shifted_assuming_bitreversed(shift, &worker)?;
...

        {
            ...
            for (w, non_res) in second_state.witness_polys_ldes[1..]
                .iter()
            ...
            for (w, perm_idx) in second_state
                .witness_polys_ldes
                .iter()
                .zip(setup.permutations_ranges[0].clone())
            {
                let poly_lde_part = w.clone_subset_assuming_bitreversed(partition_factor)?;

                let perm_part = setup.polynomial_ldes[perm_idx]
                    .clone_subset_assuming_bitreversed(partition_factor)?;
        ...
        let divisor_inversed =
            get_precomputed_inverse_divisor::<E>(required_domain_size, quotient_degree, &worker)?;
        t.mul_assign(&worker, &divisor_inversed);

        t.bitreverse_enumeration(&worker);

        let t_poly_in_monomial_form =
            t.icoset_fft_for_generator(&worker, &E::Fr::multiplicative_generator());
        ...
        println!(
            "T degree = {}",
            get_degree::<E::Fr>(&t_poly_in_monomial_form)
        );
        ...
        let state = ThirdPartialProverState::<E, H> {
            ...
            grand_product_polynomial_in_monomial_form: second_state
                .grand_product_polynomial_in_monomial_form,
            grand_product_polynomial_multioracle_tree: second_state
                .grand_product_polynomial_multioracle_tree,
    ...
    pub(crate) fn fourth_step_from_third_step(
        ...
        let mut state = FourthPartialProverState::<E, H> {
            ...
            grand_product_polynomial_in_monomial_form: third_state
                .grand_product_polynomial_in_monomial_form,
            grand_product_polynomial_multioracle_tree: third_state
                .grand_product_polynomial_multioracle_tree,
        ...
        state.grand_product_at_z =
            state.grand_product_polynomial_in_monomial_form[0].evaluate_at(&worker, z);
        state.grand_product_at_z_omega =
            state.grand_product_polynomial_in_monomial_form[0].evaluate_at(&worker, z_by_omega);
    ...
    fn perform_fri<P: Prng<E::Fr, Input = H::Output>>(
        ...
        let mut final_aggregate =
            Polynomial::from_values(vec![E::Fr::zero(); required_divisor_size])?;

        let mut precomputed_bitreversed_coset_divisor =
            Polynomial::from_values(vec![E::Fr::one(); required_divisor_size])?;
        precomputed_bitreversed_coset_divisor
            .distribute_powers(&worker, precomputed_bitreversed_coset_divisor.omega);
        ...
        for witness_request in witness_opening_requests.iter() {
            ...
            for (poly, value) in witness_request
                .polynomials
                .iter()
                .zip(witness_request.opening_values.iter())
            {
        ...
        for setup_request in setup_opening_requests.iter() {
            ...
            for ((poly, value), setup_value) in setup_request
                .polynomials
                .iter()
                .zip(setup_request.opening_values.iter())
                .zip(setup_request.setup_values.iter())
            {
                scratch_space_numerator.reuse_allocation(&poly);
                ...
                worker.scope(scratch_space_numerator.as_ref().len(), |scope, chunk| {
                    for (num, omega) in scratch_space_numerator
                        .as_mut()
                        .chunks_mut(chunk)
    ...
    pub(crate) fn fifth_step_from_fourth_step<P: Prng<E::Fr, Input = H::Output>>(
...

        {
            ...
            assert_eq!(
                setup_values.len(),
                opening_values.len(),
                "number of setup values is not equal to number of opening values"
            );
        ...
        let storages = vec![
            &fourth_state.wire_values_at_z,
            &fourth_state.wire_values_at_z_omega,
        ];
...

        {
            ...
            let grand_product_tree_params = fourth_state
                .grand_product_polynomial_multioracle_tree
                .params
                .clone();
            ...
            let grand_product_query = fourth_state
                .grand_product_polynomial_multioracle_tree
                .produce_multiquery(
                    indexes.clone(),
            ...
            let quotient_query = fourth_state
                .t_poly_parts_multioracle_tree
                .produce_multiquery(
                    indexes.clone(),
                ...
                .zip(fri_subtrees_params.iter())
            {
                ...
                let indexes: Vec<usize> =
                    (idx_start..(idx_start + params.values_per_leaf)).collect();
