...
impl<E: Engine, H: BinaryTreeHasher<E::Fr>> SetupMultioracle<E, H> {
    pub fn from_assembly<P: PlonkConstraintSystemParams<E>, MG: MainGateEquation>(
        ...
        for id in ids.iter() {
            let mut setup_poly = storage
                .remove(&id)
                .expect(&format!("must contain a poly for id {:?}", id));
            setup_poly.pad_to_domain()?;
            let coeffs = setup_poly.ifft_using_bitreversed_ntt(
                &worker,
                &omegas_inv_bitreversed,
                &E::Fr::one(),
            )?;
            mononial_forms.push(coeffs.clone());
            let lde = coeffs.bitreversed_lde_using_bitreversed_ntt(
                &worker,
                LDE_FACTOR,
                &omegas_bitreversed,
        ...
        for mut p in permutations.iter().cloned() {
            p.pad_to_domain()?;
            let coeffs =
                p.ifft_using_bitreversed_ntt(&worker, &omegas_inv_bitreversed, &E::Fr::one())?;
            mononial_forms.push(coeffs.clone());
            let lde = coeffs.bitreversed_lde_using_bitreversed_ntt(
                &worker,
                LDE_FACTOR,
        ...
        for mut selector in gate_selectors.into_iter() {
            ...
            let coeffs = selector.ifft_using_bitreversed_ntt(
                &worker,
                &omegas_inv_bitreversed,
                &E::Fr::one(),
            )?;
