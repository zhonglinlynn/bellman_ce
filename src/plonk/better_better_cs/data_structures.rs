...
pub enum PolynomialProxy<'a, E: Engine, P: PolynomialForm> {
    Borrowed(&'a Polynomial<E, P>),
    Owned(Polynomial<E, P>),
}

impl<'a, E: Engine, P: PolynomialForm> PolynomialProxy<'a, E, P> {
    pub fn from_owned(poly: Polynomial<E, P>) -> Self {
        PolynomialProxy::Owned(poly)
    }

    pub fn from_borrowed(poly: &'a Polynomial<E, P>) -> Self {
        PolynomialProxy::Borrowed(poly)
    }

    pub fn as_ref(&self) -> &Polynomial<E, P> {
        match self {
    ...
    pub fn as_data_ref_mut(&mut self) -> &mut [E::Fr] {
        match self {
    ...
    pub fn into_poly(self) -> Polynomial<E, P> {
        match self {
...
pub fn clone_as_borrowed<'a, 'b: 'a, E: Engine, P: PolynomialForm>(
    src: &'a PolynomialProxy<'b, E, P>,
) -> PolynomialProxy<'a, E, P> {
    match src {
...
pub struct AssembledPolynomialStorage<'a, E: Engine> {
    pub state_map: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Values>>,
    pub witness_map: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Values>>,
    pub setup_map: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Values>>,
    pub scratch_space:
        std::collections::HashMap<PolynomialInConstraint, PolynomialProxy<'a, E, Values>>,
    pub gate_selectors: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Values>>,
    pub named_polys: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Values>>,
    pub is_bitreversed: bool,
    pub lde_factor: usize,
}

pub struct AssembledPolynomialStorageForMonomialForms<'a, E: Engine> {
    pub state_map: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Coefficients>>,
    pub witness_map:
        std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Coefficients>>,
    pub setup_map: std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Coefficients>>,
    pub gate_selectors:
        std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Coefficients>>,
    pub named_polys:
        std::collections::HashMap<PolyIdentifier, PolynomialProxy<'a, E, Coefficients>>,
...
impl<'a, E: Engine> AssembledPolynomialStorage<'a, E> {
    pub fn get_poly(&self, id: PolyIdentifier) -> &Polynomial<E, Values> {
        match id {
            p @ PolyIdentifier::VariablesPolynomial(..) => self
                .state_map
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::WitnessPolynomial(..) => self
                .witness_map
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::GateSetupPolynomial(..) => self
                .setup_map
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::GateSelector(..) => self
                .gate_selectors
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::PermutationPolynomial(..) => self
                .setup_map
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::LookupSelector => self
                .named_polys
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::LookupTableEntriesPolynomial(..) => self
                .named_polys
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
            p @ PolyIdentifier::NamedSetupPolynomial(..) => self
                .named_polys
                .get(&p)
                .expect(&format!("poly {:?} must exist", p))
                .as_ref(),
    ...
    pub fn get_selector_for_gate(&self, gate: &dyn GateInternal<E>) -> &Polynomial<E, Values> {
    ...
    pub fn add_setup_polys<'b: 'a>(
        &mut self,
        ids: &[PolyIdentifier],
        polys: &'b [Polynomial<E, Values>],
    ) {
...
impl<'a, E: Engine> AssembledPolynomialStorageForMonomialForms<'a, E> {
    pub fn get_poly(&self, id: PolyIdentifier) -> &Polynomial<E, Coefficients> {
        match id {
            p @ PolyIdentifier::VariablesPolynomial(..) => self
                .state_map
                .get(&p)
    ...
    pub fn get_selector_for_gate(
        &self,
        gate: &dyn GateInternal<E>,
    ...
    pub fn add_setup_polys<'b: 'a>(
        &mut self,
        ids: &[PolyIdentifier],
        polys: &'b [Polynomial<E, Coefficients>],
    ) {
        assert_eq!(ids.len(), polys.len());
        for (id, poly) in ids.iter().zip(polys.iter()) {
            let proxy = PolynomialProxy::from_borrowed(poly);
...
pub struct LookupDataHolder<'a, E: Engine> {
    pub eta: E::Fr,
    pub f_poly_unpadded_values: Option<Polynomial<E, Values>>,
    pub t_poly_unpadded_values: Option<PolynomialProxy<'a, E, Values>>,
    pub t_shifted_unpadded_values: Option<PolynomialProxy<'a, E, Values>>,
    pub s_poly_unpadded_values: Option<Polynomial<E, Values>>,
    pub s_shifted_unpadded_values: Option<Polynomial<E, Values>>,
    pub t_poly_monomial: Option<PolynomialProxy<'a, E, Coefficients>>,
    pub s_poly_monomial: Option<Polynomial<E, Coefficients>>,
    pub selector_poly_monomial: Option<PolynomialProxy<'a, E, Coefficients>>,
    pub table_type_poly_monomial: Option<PolynomialProxy<'a, E, Coefficients>>,
}
