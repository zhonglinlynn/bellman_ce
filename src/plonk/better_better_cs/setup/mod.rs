...
pub struct Setup<E: Engine, C: Circuit<E>> {
    ...
    pub gate_setup_monomials: Vec<Polynomial<E, Coefficients>>,
    pub gate_selectors_monomials: Vec<Polynomial<E, Coefficients>>,
    pub permutation_monomials: Vec<Polynomial<E, Coefficients>>,

    pub total_lookup_entries_length: usize,
    pub lookup_selector_monomial: Option<Polynomial<E, Coefficients>>,
    pub lookup_tables_monomials: Vec<Polynomial<E, Coefficients>>,
    pub lookup_table_type_monomial: Option<Polynomial<E, Coefficients>>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Setup")
            ...
            .field(
                ...
                &self.total_lookup_entries_length,
            )
...
impl<E: Engine, C: Circuit<E>> VerificationKey<E, C> {
    pub fn from_setup(
        ...
        for (p, c) in vec![
            ...
            (
                &setup.gate_selectors_monomials,
                &mut new.gate_selectors_commitments,
            ),
            (
                &setup.permutation_monomials,
                &mut new.permutation_commitments,
            ),
            (
                &setup.lookup_tables_monomials,
                &mut new.lookup_tables_commitments,
            ),
        ]
        .into_iter()
        {
...
impl<'a, E: Engine> AssembledPolynomialStorageForMonomialForms<'a, E> {
    pub fn extend_from_setup<C: Circuit<E>>(
        &mut self,
        setup: &'a Setup<E, C>,
        ...
        println!("has_selectors: {:?}", has_selectors);
        ...
        for gate in all_gates.iter() {
            for poly_id in gate.setup_polynomials().into_iter() {
                let poly_ref = setup_gates_iter.next().expect(&format!(
                    "must have gate setup poly {:?} for gate {:?} in setup",
                    poly_id, gate
                ));
        ...
        if has_selectors {
            ...
            for gate in all_gates.into_iter() {
                ...
                let poly_ref = selector_iter.next().expect(&format!(
                    "must have gate selector poly for gate {:?} in setup",
                    gate
                ));
                let proxy = PolynomialProxy::from_borrowed(poly_ref);
