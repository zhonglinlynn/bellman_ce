...
pub trait MainGateEquation: GateEquation {
    ...
    fn format_term<E: Engine>(
        instance: MainGateTerm<E>,
        padding: Variable,
    ) -> Result<(Vec<Variable>, Vec<E::Fr>), SynthesisError>;
}

pub trait GateEquation:
    GateEquationInternal + Sized + Clone + std::hash::Hash + std::default::Default
{
...
pub trait GateEquationInternal: Send + Sync + 'static + std::any::Any + std::fmt::Debug {
    fn degree(&self) -> usize;
...
impl PartialEq for dyn GateEquationInternal {
    fn eq(&self, other: &Self) -> bool {
        self.degree() == other.degree() && self.get_constraints() == other.get_constraints()
    }
}
...
impl Width4MainGateWithDNextEquation {
    pub fn get_equation() -> Self {
        ...
        for i in 0..4 {
            // N * Q_n
            terms.push(PolynomialMultiplicativeTerm(
                Coefficient::PlusOne,
                vec![
        ...
        terms.push(PolynomialMultiplicativeTerm(
            Coefficient::PlusOne,
            vec![PolynomialInConstraint::SetupPolynomial(
                "main gate of width 4",
                5,
                TimeDilation(0),
            )],
...
fn check_gate_is_allowed_for_params<
    E: Engine,
...
pub trait ConstraintSystem<E: Engine> {
    ...
    fn new_single_gate_for_trace_step<G: GateEquation>(
        &mut self,
        equation: &G,
    ...
    fn new_gate_in_batch<G: GateEquation>(
        &mut self,
        equation: &G,
        coefficients_assignments: &[E::Fr],
        variables_assignments: &[Variable],
...
//     pub state_map: std::collections::HashMap<PolyIdentifier, Polynomial::<E, Values>>,
//     pub witness_map: std::collections::HashMap<PolyIdentifier,Polynomial::<E, Values>>,
//     pub setup_map: std::collections::HashMap<PolyIdentifier, Polynomial::<E, Values>>,
// }
...
pub struct GateConstantCoefficientsStorage<E: Engine>(
    pub std::collections::HashMap<Box<dyn GateEquationInternal>, Vec<E::Fr>>,
);
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>, MG: MainGateEquation> ConstraintSystem<E>
    for TrivialAssembly<E, P, MG>
{
    type Params = P;
    type MainGate = MG;
    ...
    fn new_gate_in_batch<G: GateEquation>(
        ...
        debug_assert!(
            check_gate_is_allowed_for_params::<E, P, G>(&gate),
            "supplied params do not work with gate {:?}",
            gate
        );
        ...
        if let Some(tracker) = self
            .aux_gate_density
            .0
            .get_mut(gate.as_internal() as &dyn GateEquationInternal)
        {
            if tracker.len() != n {
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>, MG: MainGateEquation> TrivialAssembly<E, P, MG> {
    ...
    fn add_gate_into_list<G: GateEquation>(&mut self, gate: &G) {
        if !self
            .constraints
            .contains(gate.as_internal() as &dyn GateEquationInternal)
        {
            ...
            self.gate_internal_coefficients
                .0
                .insert(Box::from(gate.clone().into_internal()), gate_constants);
        }
    }
    ...
    pub fn new() -> Self {
        ...
        assert!(
            check_gate_to_support_public_inputs(&tmp.main_gate),
            "default gate must support making public inputs"
        );
    ...
    pub fn finalize(&mut self) {
        ...
        let (vars, coeffs) =
            MG::format_term(empty_gate, dummmy).expect("must make empty padding gate");

        for _ in self.n()..(self.n().next_power_of_two() - 1) {
            self.new_single_gate_for_trace_step(MG::static_description(), &coeffs, &vars, &[])
    ...
    pub fn is_satisfied(&self) -> bool {
        ...
        for (gate_type, density) in self.aux_gate_density.0.iter() {
            ...
            let constants = self
                .gate_internal_coefficients
                .0
                .get(gate_type.as_ref())
                .expect(&format!(
                    "must get internal coefficients for the gate {:?}",
                    gate_type.as_ref()
                ));
                        ...
                        for poly in term.1.iter() {
                            let value = match poly {
                                PolynomialInConstraint::VariablesPolynomial(
                                    ...
                                    debug_assert!(
                                        *dilation <= 1,
                                        "only handles variables on this and next step"
                                    );
                                    if trace_index == n && *dilation > 0 {
                                        continue;
                                    }
                                    let variable =
                                        storage.get_variable(poly, gate_index).expect(&format!(
                                            "must get a variable for poly {:?} for aux gate {}",
                                            poly, gate_index
                                        ));
                                    let value = self
                                        .get_value(variable)
                                        .expect("must get a state variable value");
                                ...
                                PolynomialInConstraint::SetupPolynomial(
                                    ...
                                    debug_assert_eq!(
                                        *dilation, 0,
                                        "only handles setup polys on this step for now"
                                    );
                                    if trace_index == n && *dilation > 0 {
                                        continue;
                                    }
                                    ...
                                    let value =
                                        storage.get_value(poly, gate_index).expect(&format!(
                                            "must get a setup value for poly {:?} for aux gate {}",
                                            poly, gate_index
                                        ));

                                    value
                                }
                                PolynomialInConstraint::WitnessPolynomial(
                                    _,
                                    TimeDilation(dilation),
                                ) => {
                                    debug_assert_eq!(
                                        *dilation, 0,
                                        "only handles witness polys on this step for now"
                                    );
                                    if trace_index == n && *dilation > 0 {
                                        continue;
                                    }
                                    ...
                                    let value =
                                        storage.get_value(poly, gate_index).expect(&format!(
                                        "must get a witness value for poly {:?} for aux gate {}",
                                        poly, gate_index
                                    ));
    ...
    pub(crate) fn make_permutations(&self, worker: &Worker) -> Vec<Polynomial<E, Values>> {
    ...
    pub fn output_gate_selectors(
        ...
        let one = E::Fr::one();
        let empty_poly =
            Polynomial::<E, Values>::new_for_size(self.n().next_power_of_two(), worker)?;
...
pub fn prove_with_rescue_bn256<
    P: PlonkConstraintSystemParams<bn256::Bn256>,
    MG: MainGateEquation,
    C: Circuit<bn256::Bn256>,
>(
    circuit: &C,
) -> Result<(), SynthesisError> {
    use super::*;
    ...
    use crate::plonk::commitments::transcript::{rescue_transcript, Prng};
    ...
    let (setup_multioracle, mut permutations) =
        SetupMultioracle::from_assembly(assembly, hasher.clone(), &worker)?;
    ...
    let (prover, first_state, first_message) =
        RedshiftProver::first_step(assembly, hasher.clone(), &worker)?;
    ...
    let first_verifier_message = FirstVerifierMessage::<bn256::Bn256> { beta, gamma };
    ...
    let second_verifier_message = SecondVerifierMessage::<bn256::Bn256> { alpha, beta, gamma };
    ...
    let wire_values_at_z_omega: Vec<_> =
        wire_values_at_z_omega.into_iter().map(|el| el.1).collect();

    for w in wire_values_at_z
        .iter()
...
#[cfg(feature = "redshift")]
pub fn prove_with_poseidon_bn256<
    P: PlonkConstraintSystemParams<bn256::Bn256>,
    MG: MainGateEquation,
    C: Circuit<bn256::Bn256>,
>(
    ...
    use crate::plonk::commitments::transcript::{poseidon_transcript, Prng};
...
#[cfg(feature = "redshift")]
pub fn prove_with_hash_counting_bn256<
    P: PlonkConstraintSystemParams<bn256::Bn256>,
    MG: MainGateEquation,
    C: Circuit<bn256::Bn256>,
>(
...
mod test {
    ...
    impl<E: Engine> Circuit<E> for TestCircuit4<E> {
        fn synthesize<CS: ConstraintSystem<E>>(&self, cs: &mut CS) -> Result<(), SynthesisError> {
            ...
            let gamma = cs.alloc_input(|| Ok(E::Fr::from_str("20").unwrap()))?;
    ...
    fn test_make_setup_for_trivial_circuit() {
        ...
        let (setup_multioracle, mut permutations) =
            SetupMultioracle::from_assembly(assembly, hasher.clone(), &worker).unwrap();
        ...
        let (prover, first_state, first_message) =
            RedshiftProver::first_step(assembly, hasher.clone(), &worker).unwrap();
        ...
        let (second_state, second_message) = prover
            .second_step_from_first_step(
                first_state,
                first_verifier_message,
                &permutations,
                &worker,
        ...
        let (third_state, third_message) = prover
            .third_step_from_second_step(
                second_state,
                second_verifier_message,
                &setup_multioracle,
                &worker,
        ...
        let (fourth_state, fourth_message) = prover
            .fourth_step_from_third_step(
                third_state,
                third_verifier_message,
                &setup_multioracle,
                &worker,
